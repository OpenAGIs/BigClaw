...
- The residual operator wrappers are shell aliases that proxy into `scripts/ops/bigclawctl`.
- Behavioral ownership now lives in Go under `bigclaw-go/cmd/bigclawctl`.
- Legacy Python validation is reduced to the frozen source-level compile check.
...
These shell shims should remain until operator docs and external automation references are updated
to invoke `bash scripts/ops/bigclawctl ...` directly. The repo-root Python shims and the residual
`scripts/ops/*.py` wrappers were removed because the root no longer carries Python
packaging/bootstrap ownership.
...
- Root compatibility retirement:
  repo operators must stop invoking removed repo-root and `scripts/ops/*.py` Python shims and switch to
...
- PR description focus:
  - migrated entrypoints and retired Python wrappers
