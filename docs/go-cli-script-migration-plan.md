...
subcommands, then retire the transitional Python wrappers once operator docs and verification
paths are Go-first.
...
The Python wrapper layer has been removed. The retained shell aliases stay only as lightweight
operator shortcuts over `bash scripts/ops/bigclawctl ...`.
...
- Workspace/refill cutover:
  ...
  must stay deleted and operator validation should use `bash scripts/ops/bigclawctl ...`.
