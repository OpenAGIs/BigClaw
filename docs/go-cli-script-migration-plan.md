...
subcommands and retire the packaging-era Python wrapper files once operator docs are updated.
...
The repo-root Python wrappers have been removed because the root no longer carries Python
packaging/bootstrap ownership. Remaining compatibility wrappers are the bash aliases above.
...
- Update repo docs that still present compatibility wrappers instead of `scripts/ops/bigclawctl`
  as the primary path.
...
- Root compatibility retirement:
  repo operators must invoke `bash scripts/ops/bigclawctl ...` directly for GitHub sync, refill,
  and workspace flows.
