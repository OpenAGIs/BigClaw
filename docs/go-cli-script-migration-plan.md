...
subcommands. The former root and ops Python shim layer has now been removed after operator
docs moved to the Go CLI entrypoints.
...
- Behavioral ownership lives in Go under `bigclaw-go/cmd/bigclawctl`.
...
### Compatibility wrappers still kept in place

- `scripts/ops/bigclaw-symphony`
- `scripts/ops/bigclaw-issue`
- `scripts/ops/bigclaw-panel`

The root and ops Python shim files were removed once operator docs moved to
...
- Continue retiring migration-only Python surfaces outside this root-script batch.
...
- Workspace CLI compatibility:
  `bigclawctl workspace` and `bigclawctl workspace validate` remain the supported operator paths.
...
- Operator docs:
  repo instructions must not imply that the retired Python root/ops scripts still exist.
