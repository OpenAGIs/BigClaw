...
subcommands and keep the repository root on Go-owned entrypoints.
...
These shims remain only as shell aliases while the canonical operator path stays
...
- Keep repo docs and automation on Go-only root entrypoints.
...
- Root compatibility retirement:
  repo operators must stop invoking removed repo-root Python entrypoints and switch to
