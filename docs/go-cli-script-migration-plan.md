...
subcommands, while preserving operator behavior on the Go CLI path during the cutover window.
...
invoke `bash scripts/ops/bigclawctl ...` directly. The repo-root Python ops shims were removed
because the repository no longer carries Python ownership for those entrypoints.
