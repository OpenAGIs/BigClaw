...
- Repo-root Python compatibility is now limited to migration source assets under `src/bigclaw` plus targeted validation coverage.
...
These shell wrappers should remain until operator docs and external automation references are
fully normalized on `bash scripts/ops/bigclawctl ...`. The Python ops wrappers have now been
removed because the root no longer carries Python packaging/bootstrap ownership.
