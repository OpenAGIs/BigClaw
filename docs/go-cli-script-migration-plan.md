...
subcommands and retire the legacy Python operator wrappers once tracked repo surfaces have
switched to the Go entrypoints.
...
These shell aliases should remain until operator docs and external automation references finish
converging on `bash scripts/ops/bigclawctl ...` directly. The Python operator wrappers are retired
...
- Root compatibility retirement:
  repo operators must stop invoking removed Python wrappers and switch to
