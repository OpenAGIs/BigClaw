...
- deleted `scripts/ops/bigclaw_refill_queue.py`; use `bigclawctl refill`
- deleted `scripts/ops/bigclaw_workspace_bootstrap.py`; use `bigclawctl workspace ...`
- deleted `scripts/ops/symphony_workspace_bootstrap.py`; use `bigclawctl workspace ...`
- deleted `scripts/ops/symphony_workspace_validate.py`; use `bigclawctl workspace validate`
...
The retained shell wrappers should remain until operator docs and external automation references are
updated to invoke `bash scripts/ops/bigclawctl ...` directly. The `scripts/ops` Python wrappers are
deleted because root operator ownership now lives in Go under `bigclawctl`.
...
- Root compatibility retirement:
  repo operators must stop invoking removed `scripts/ops` Python wrappers and switch to
