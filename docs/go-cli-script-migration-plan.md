...
- `bigclaw-go/scripts/benchmark/soak-local` -> `bigclawctl automation benchmark soak-local`
...
- Shell compatibility wrappers under `scripts/ops/` only translate legacy flags/defaults before dispatching into `scripts/ops/bigclawctl`.
