...
- `workflow.md`, `.githooks/post-commit`, and `.githooks/post-rewrite` now invoke the Go-first toolchain by default while the remaining shell wrapper names exist only as compatibility shims over `scripts/ops/bigclawctl`
- at the time this slice was defined, the remaining Python-named wrappers still existed as migration shims and `BIG-GOM-308` was the planned follow-on slice to remove Python from the default operator path
