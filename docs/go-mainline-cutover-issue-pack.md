...
- former repo-root Python shim entrypoints for github-sync, workspace bootstrap, and refill
...
- `workflow.md`, `.githooks/post-commit`, and `.githooks/post-rewrite` now invoke the Go-first toolchain by default
- the former repo-root Python wrappers were removed once root entrypoints switched fully to `scripts/ops/bigclawctl`
