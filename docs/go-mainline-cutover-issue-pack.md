...
- `workflow.md`, `.githooks/post-commit`, and `.githooks/post-rewrite` now invoke the Go-first toolchain by default through non-Python shell entrypoints
- the earlier Python-named operator compatibility wrappers have been removed from the default operator path, leaving only migration-reference Python surfaces outside the active Go-first toolchain
