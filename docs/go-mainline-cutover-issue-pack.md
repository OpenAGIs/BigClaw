...
- retired `src/bigclaw/github_sync.py`
...
- retired `src/bigclaw/parallel_refill.py`
- compatibility wrappers `scripts/ops/bigclaw_github_sync`, `scripts/ops/symphony_workspace_bootstrap`, and `scripts/ops/bigclaw_refill_queue`
...
- `workflow.md`, `.githooks/post-commit`, and `.githooks/post-rewrite` now invoke the Go-first toolchain by default through compatibility wrappers that no longer use `.py` entrypoints
- the historical Python module shims have now been removed from `src/bigclaw/`, leaving the Go-first operator path and shell wrappers as the only maintained surfaces
...
Python source:
- collaboration helpers now live in `src/bigclaw/observability.py`
- retired `src/bigclaw/repo_board.py`
- retired `src/bigclaw/repo_commits.py`
- retired `src/bigclaw/repo_gateway.py`
- retired `src/bigclaw/repo_governance.py`
