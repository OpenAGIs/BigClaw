...
- Current repo tranche status as of March 25, 2026:
  - active slices: `BIG-PAR-309` — Add product regression coverage for ClawHost workflow review reasons
  - standby slices: none
  - recently completed slices: `BIG-PAR-301` — Reuse one ClawHost snapshot across debug status surfaces; `BIG-PAR-302` — Add team and project scoping to ClawHost debug surfaces; `BIG-PAR-303` — Add team and project scoping to ClawHost fleet surface; `BIG-PAR-304` — Harden scoped ClawHost expansion export regressions; `BIG-PAR-305` — Add product regression coverage for ClawHost fleet filtering; `BIG-PAR-306` — Add product regression coverage for scoped ClawHost recovery scorecards; `BIG-PAR-307` — Add product regression coverage for scoped ClawHost workflow lanes; `BIG-PAR-308` — Add product regression coverage for scoped ClawHost rollout planners
