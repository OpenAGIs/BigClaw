...
- Current repo tranche status as of March 24, 2026:
  - active slices: none
  - standby slices: none
  - recently completed slices: `BIG-PAR-320` — Add product regression coverage for ClawHost fleet compatibility aliases; `BIG-PAR-321` — Add product regression coverage for default ClawHost fleet compatibility alias; `BIG-PAR-322` — Add product regression coverage for ClawHost workflow parsing defaults; `BIG-PAR-323` — Add product regression coverage for ClawHost workflow pairing defaults; `BIG-PAR-324` — Add product regression coverage for ClawHost workflow metadata precedence; `BIG-PAR-325` — Add product regression coverage for ClawHost workflow lane compatibility alias; `BIG-PAR-326` — Expose ClawHost workflow report filters and operational signals; `BIG-PAR-327` — Expose ClawHost rollout report filters
