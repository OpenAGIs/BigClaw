...
- Current repo tranche status as of March 24, 2026:
  - active slices: none
  - standby slices: none
  - recently completed slices: `BIG-PAR-340` — Add policy filter assertions to standalone API regressions; `BIG-PAR-341` — Add scoped debug policy filter assertion; `BIG-PAR-342` — Pin default ClawHost product filter maps in regressions; `BIG-PAR-343` — Pin default fleet filter maps in regressions; `BIG-PAR-344` — Add policy report fallback filter regression; `BIG-PAR-345` — Pin workflow-lane idle filter maps in regressions; `BIG-PAR-346` — Add policy surface empty-state unit coverage; `BIG-PAR-347` — ClawHost expansion endpoints preserve default unscoped reports
