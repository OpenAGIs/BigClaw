...
- Current repo tranche status as of March 25, 2026:
  - active slices: `BIG-PAR-379` — Add bootstrap joinLines helper coverage
  - standby slices: none
  - recently completed slices: `BIG-PAR-371` — Add saved-view digest recipient helper coverage; `BIG-PAR-372` — Add saved-view audit visibility and ordering coverage; `BIG-PAR-373` — Broaden direct recovery builder branch coverage; `BIG-PAR-374` — Add billing usage unit coverage; `BIG-PAR-375` — Add expansion helper unit coverage; `BIG-PAR-376` — Add event stream helper unit coverage; `BIG-PAR-377` — Add checkpoint error helper coverage; `BIG-PAR-378` — Add bootstrap helper unit coverage
