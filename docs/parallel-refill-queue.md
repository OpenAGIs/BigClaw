...
- Current repo tranche status as of March 24, 2026:
  - active slices: none
  - standby slices: none
  - recently completed slices: `BIG-PAR-328` — Expose ClawHost recovery report filters; `BIG-PAR-329` — Expose ClawHost policy report filters; `BIG-PAR-330` — Expose ClawHost fleet report filters; `BIG-PAR-331` — Add filters to ClawHost policy surface payload; `BIG-PAR-332` — Add filters to ClawHost rollout surface payload; `BIG-PAR-333` — Add filters to ClawHost recovery surface payload; `BIG-PAR-334` — Add filters to ClawHost readiness surface payload; `BIG-PAR-335` — Thread filters into ClawHost workflow surface payload
