...
- Current repo tranche status as of March 25, 2026:
  - active slices: `BIG-PAR-412` — Add refill runnable-count edge coverage; `BIG-PAR-411` — Add local tracker state-update edge coverage
  - standby slices: none
  - recently completed slices: `BIG-PAR-403` — Add local tracker accessor coverage; `BIG-PAR-404` — Add refill queue selection helper coverage; `BIG-PAR-405` — Add local tracker decode helper coverage; `BIG-PAR-406` — Add refill queue status sync coverage; `BIG-PAR-407` — Add local tracker read/load coverage; `BIG-PAR-408` — Add refill queue load/save helper coverage; `BIG-PAR-409` — Add local tracker save/write-path coverage; `BIG-PAR-410` — Add refill markdown helper coverage
