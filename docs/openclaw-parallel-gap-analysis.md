...
- `OPE-254` / `BIG-PAR-088` tracks the remaining rollback safeguard caveats in `bigclaw-go/docs/reports/rollback-safeguard-follow-up-digest.md`.
...
- `BIG-GO-903` publishes `bigclaw-go/docs/reports/test-harness-migration-plan.md` as the executable split for legacy pytest coverage into Go package tests, regression/golden checks, and integration harness lanes.
