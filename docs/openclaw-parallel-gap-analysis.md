...
- `OPE-264` / `BIG-PAR-094` tracks the distributed trace export bundle path in `bigclaw-go/internal/api/distributed.go` and the remaining external tracing backend and span-propagation caveats in `bigclaw-go/docs/reports/tracing-backend-follow-up-digest.md`.
