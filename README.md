...
- The only remaining repo-root shell helpers are `scripts/dev_bootstrap.sh` and
  `scripts/ops/bigclawctl`. Invoke `issue`, `panel`, and `symphony` directly
  through `bash scripts/ops/bigclawctl ...`.
...
`scripts/ops/bigclawctl`; retired root Python ops wrappers and transitional
alias wrappers should stay deleted and GitHub sync is Go/shell-only via
