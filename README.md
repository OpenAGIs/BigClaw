...
Python surface must be exercised, run the Go-native migration coverage that
replaced the deleted Python test suites:

```bash
cd bigclaw-go
go test ./internal/planning ./internal/regression
```

Or use the bootstrap helper to validate Go first and then run the migration
planning coverage from the active environment without editable install
