...
- The unattended workflow path is Go/shell-only: use `bash scripts/ops/bigclawctl workspace ...`,
  `bash scripts/ops/bigclawctl github-sync ...`, and `bash scripts/ops/bigclawctl refill ...`.
...
the Go-first operator entrypoint is `scripts/ops/bigclawctl`; the unattended
workflow bootstrap/sync/refill path is now Go/shell-only.
