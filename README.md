...
- `python3 bigclaw-go/scripts/benchmark/soak_local.py` remains a compatibility
  shim into `bigclawctl automation ...`, while the deleted
  `bigclaw-go/scripts/e2e/run_task_smoke.py` and
  `bigclaw-go/scripts/migration/shadow_compare.py` entrypoints now map directly
  to `go run ./bigclaw-go/cmd/bigclawctl automation ...`; the migration matrix lives in
