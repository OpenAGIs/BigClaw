...
- Refill and workspace operations are Go-only: use
...
the Go-first operator entrypoint is `scripts/ops/bigclawctl`; refill,
workspace bootstrap, workspace validation, and GitHub sync are all Go/shell-only
via `scripts/ops/bigclawctl`.
