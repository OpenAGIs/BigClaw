...
have been reduced to the frozen compatibility file `src/bigclaw/__init__.py`,
which is not packaged from the repository root.
...
Python surface must be exercised, validate the surviving package root directly
from source:
...
`cd bigclaw-go && go test ./internal/bootstrap`, and finally
`python3 -m py_compile src/bigclaw/__init__.py`.
