...
- Use `bash scripts/ops/bigclawctl refill ...`,
  `bash scripts/ops/bigclawctl workspace bootstrap ...`, and
  `bash scripts/ops/bigclawctl workspace validate ...` directly.
...
the Go-first operator entrypoint is `scripts/ops/bigclawctl`, including refill
and workspace flows; GitHub sync is also Go/shell-only via `scripts/ops/bigclawctl`.
