            ...
            CandidateEntry(
                ...
                evidence_links=[
                    EvidenceLink(
                        ...
                        note="component inventory, accessibility, UI acceptance, and execution governance coverage",
                    ),
                    EvidenceLink(
...
# Legacy observability surface migrated from observability.py
...
class CollaborationThread:
    ...
    def mention_count(self) -> int:
        return sum(len(comment.mentions) for comment in self.comments) + sum(len(decision.mentions) for decision in self.decisions)
...
# Legacy UI review surface migrated from ui_review.py
...
# Legacy runtime, workflow, reporting, and operations surface migrated from runtime.py
...
_legacy_reports_surface = sys.modules[__name__]
_legacy_design_system_surface = sys.modules[__name__]
sys.modules[f"{__name__}.design_system"] = sys.modules[__name__]
globals()["design_system"] = sys.modules[__name__]
