...
_runtime_exports_before = set(globals())
...
_runtime_export_names = sorted(
    name for name in (set(globals()) - _runtime_exports_before)
    if name != "_runtime_exports_before"
)
for _runtime_export_name in _runtime_export_names:
    _runtime_export = globals()[_runtime_export_name]
    if getattr(_runtime_export, "__module__", None) == __name__:
        _runtime_export.__module__ = f"{__name__}.runtime"

_install_compat_module(
    _compat_source,
    "runtime",
    _runtime_export_names,
    GO_MAINLINE_REPLACEMENT=GO_MAINLINE_REPLACEMENT,
)

_legacy_runtime_surface = sys.modules[f"{__name__}.runtime"]
