  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .kicker {{
      ...
      font: 600 0.7rem/1.2 "SFMono-Regular", Consolas, "Liberation Mono", monospace;
      text-transform: uppercase;
...
<body>
  <main>
    <div class="shell">
      ...
      <nav class="tabs" aria-label="Run detail sections">{tab_buttons}</nav>
  ...
  <script>
    ...
    const renderEvent = (eventId) => {{
      const selected = timelineData.find((event) => event.id === eventId);
      if (!selected || !detailTitle || !detailMeta || !detailSummary || !detailList) return;
      detailTitle.textContent = selected.title;
      detailMeta.textContent = `${{selected.lane}} / ${{selected.status}} / ${{selected.timestamp}}`;
      detailSummary.textContent = selected.summary;
      detailList.innerHTML = '';
      const details = selected.details && selected.details.length ? selected.details : ['No additional details.'];
      ...
      timelineButtons.forEach((button) => {{
        button.classList.toggle('active', button.dataset.eventId === eventId);
      }});
    }};
