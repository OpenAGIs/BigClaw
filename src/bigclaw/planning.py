...
def render_scope_freeze_report(board: ScopeFreezeBoard, audit: ScopeFreezeAudit) -> str:
    ...
    if not board.backlog_items:
        lines.append("- No backlog items recorded.")
    for item in board.backlog_items:
        lines.append(
            "- "
            f"{item.issue_id}: {item.title} phase={item.phase} "
            f"owner={item.owner or 'unassigned'} status={item.status} scope={item.scope_status} "
            f"ready={item.governance_ready}"
        )
    lines.extend(
        [
            "",
            "## Audit",
            "",
            f"- Duplicate issue ids: {', '.join(audit.duplicate_issue_ids) or 'none'}",
            f"- Missing owners: {', '.join(audit.missing_owners) or 'none'}",
            f"- Missing acceptance: {', '.join(audit.missing_acceptance) or 'none'}",
            f"- Missing validation: {', '.join(audit.missing_validation) or 'none'}",
            (
            ...
            f"- Unauthorized scope changes: {', '.join(audit.unauthorized_scope_changes) or 'none'}",
            f"- Invalid scope statuses: {', '.join(audit.invalid_scope_statuses) or 'none'}",
            f"- Unapproved exceptions: {', '.join(audit.unapproved_exceptions) or 'none'}",
        ]
    )
    return "\n".join(lines)
