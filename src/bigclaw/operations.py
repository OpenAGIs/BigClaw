...
def render_run_replay_index_page(
    ...
    <section class="surface">
      ...
      <ul>
        {''.join(f'<li>{escape(item)}</li>' for item in replay.mismatches) or '<li>No mismatches recorded.</li>'}
      </ul>
    </section>
    """
    return render_run_detail_console(
        page_title=f"Run Detail Index · {case_id}",
        eyebrow="Run Detail Index",
        hero_title=f"Run Detail Index · {case_id}",
        hero_summary="Single landing page that links the execution report, detailed run page, and replay evidence for a benchmark case.",
        stats=[
            RunDetailStat("Case ID", case_id),
            RunDetailStat("Task ID", record.run.task_id),
            RunDetailStat("Medium", record.decision.medium, tone=status_tone),
            RunDetailStat("Status", record.run.status, tone=status_tone),
            RunDetailStat("Replay", "matched" if replay.matched else "mismatch", tone=status_tone),
        ...
        tabs=[
            ...
            RunDetailTab(
                ...
                render_timeline_panel(
                    "Timeline / Log Sync",
                    "Merged run telemetry, acceptance criteria, and replay mismatches for the benchmark case.",
                    run_events,
                ),
            ),
            ...
            RunDetailTab(
                ...
                render_resource_grid(
                    "Reports",
                    "Linked outputs for the benchmark run, detailed run page, and replay page.",
                    execution_resources,
                ),
            ),
        ],
