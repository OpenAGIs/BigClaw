...
- `OPE-175`
  ...
  - Follow-up digests capture the remaining cross-process coordination and validation bundle continuation caveats in `docs/reports/cross-process-coordination-boundary-digest.md` and `docs/reports/validation-bundle-continuation-digest.md`, with the current multi-bundle lineage summarized in `docs/reports/validation-bundle-continuation-scorecard.json`, the current gate result captured in `docs/reports/validation-bundle-continuation-policy-gate.json`, and the current reviewer-facing gate detail surfaced inline in `docs/reports/live-validation-summary.json` and `docs/reports/live-validation-index.md`.
...
- `OPE-271` / `BIG-PAR-082` — validation bundle continuation caveats are consolidated in `docs/reports/validation-bundle-continuation-digest.md`, with the latest multi-bundle lineage summarized in `docs/reports/validation-bundle-continuation-scorecard.json`, the latest gate result captured in `docs/reports/validation-bundle-continuation-policy-gate.json`, and the same gate detail surfaced inline for reviewers in `docs/reports/live-validation-summary.json` and `docs/reports/live-validation-index.md`.
