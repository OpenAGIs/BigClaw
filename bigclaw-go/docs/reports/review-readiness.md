...
- A reviewer entrypoint for the completed ClawHost tranche now exists in `docs/reports/clawhost-control-plane-review-index.md`, linking the merged fleet, tenant-policy, validation, rollout, workflow, readiness, and recovery surfaces from PR `#186` together with the aggregate `/v2/control-center` regression coverage that keeps that reviewer bundle pinned.
