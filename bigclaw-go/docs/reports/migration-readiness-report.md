...
- Shadow comparison matrix across operator-supplied fixtures via `go run ./cmd/bigclawctl automation migration shadow-matrix ...`
...
- A checked-in corpus-coverage scorecard embedded in `docs/reports/shadow-matrix-report.json`
...
- Matrix now accepts anonymized corpus manifests, but production-weighted evidence still depends on operator-supplied fixture tasks and corpus slices; see `docs/reports/production-corpus-migration-coverage-digest.md`.
