...
- The concrete Python `runtime` / `scheduler` / `orchestration` /
  `workflow` retirement sequence is now tracked in
  `docs/reports/runtime-scheduler-orchestration-migration-plan.md`, including
  first-batch test ports, script conversion order, validation commands, branch
  strategy, and regression ownership.
