...
- `OPE-183` / `BIG-GO-008`
  ...
  - Follow-up takeover caveats are tracked in `docs/reports/subscriber-takeover-executability-follow-up-digest.md`, which now captures the deterministic local harness, the live two-node proof, and the remaining shared-durable ownership gap
- `OPE-184` / `BIG-GO-009`
...
- No dedicated leader-election layer exists yet; current evidence is limited to a `live_proven` local two-node shared-SQLite coordination proof captured in `docs/reports/multi-node-coordination-report.md`, while takeover and replay semantics remain `harness_proven` and broker-backed ownership remains `contract_only` in `docs/reports/cross-process-coordination-capability-surface.json`.
