...
- `OPE-265` / `BIG-PAR-095` — telemetry pipeline, sampling policy, and high-cardinality caveats are consolidated in `docs/reports/telemetry-pipeline-controls-follow-up-digest.md`, with the current machine-checkable surface in `docs/reports/telemetry-sampling-cardinality-evidence-pack.json`.
