...
- `go run ./cmd/bigclawctl e2e subscriber-takeover-fault-matrix` generates the deterministic local harness report.
...
- `go run ./cmd/bigclawctl e2e multi-node-shared-queue` now generates both the shared-queue report and the live takeover companion report in one run.
...
- Keep this digest aligned with `docs/reports/multi-subscriber-takeover-validation-report.md`, `docs/reports/multi-subscriber-takeover-validation-report.json`, `cmd/bigclawctl/e2e.go`, and `docs/reports/event-bus-reliability-report.md`.
- Keep the live companion proof references aligned with `docs/reports/live-multi-node-subscriber-takeover-report.json`, `docs/reports/live-multi-node-subscriber-takeover-artifacts/`, and `go run ./cmd/bigclawctl e2e multi-node-shared-queue`.
