...
- `scripts/e2e/subscriber_takeover_fault_matrix/main.go` generates the deterministic local harness report.
...
- `scripts/e2e/multi_node_shared_queue/main.go` now generates both the shared-queue report and the live takeover companion report in one run.
...
- Keep this digest aligned with `docs/reports/multi-subscriber-takeover-validation-report.md`, `docs/reports/multi-subscriber-takeover-validation-report.json`, `scripts/e2e/subscriber_takeover_fault_matrix/main.go`, and `docs/reports/event-bus-reliability-report.md`.
- Keep the live companion proof references aligned with `docs/reports/live-multi-node-subscriber-takeover-report.json`, `docs/reports/live-multi-node-subscriber-takeover-artifacts/`, and `scripts/e2e/multi_node_shared_queue/main.go`.
