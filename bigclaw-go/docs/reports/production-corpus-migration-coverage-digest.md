...
- `docs/reports/live-shadow-drift-rollup.json` captures the latest bundled parity/freshness rollup alongside the checked-in matrix evidence.
- `docs/migration-shadow.md` documents the current single-run and matrix workflow plus the operator-supplied corpus-manifest input boundary.
...
- There is still no real production issue/task corpus coverage checked into the repo; operators must supply operator-supplied anonymized manifests for tenant skew, large issue sets, or historical long-tail task mixes.
...
- Any operator-supplied anonymized manifest is still illustrative point-in-time evidence; it is not a live or automatically refreshed production export.
