...
- `go run ./cmd/bigclawctl migration validation-continuation-scorecard` regenerates the scorecard from checked-in bundle summaries and shared-queue evidence.
...
- `go run ./cmd/bigclawctl migration validation-continuation-policy-gate` evaluates the scorecard as a repo-native policy gate.
...
- The repo-native policy gate now refreshes automatically during `run_all.sh` closeout through `bigclawctl migration`, but enforcement is not enabled by default across ordinary workflows.
