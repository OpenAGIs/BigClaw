...
- `go run ./cmd/bigclawctl automation e2e validation-bundle-continuation-scorecard` regenerates the scorecard from checked-in bundle summaries and shared-queue evidence.
...
- `go run ./cmd/bigclawctl automation e2e validation-bundle-continuation-policy-gate` evaluates the scorecard as a repo-native policy gate.
...
- The repo-native policy gate now refreshes automatically during `run_all.sh` closeout, but most workflows still need explicit mode selection to fail closed.
