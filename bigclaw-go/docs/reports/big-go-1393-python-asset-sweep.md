# BIG-GO-1393 Python Asset Sweep

## Scope

Go-only refill lane `BIG-GO-1393` records the remaining Python asset baseline with explicit focus on `src/bigclaw`, `tests`, `scripts`, and `bigclaw-go/scripts`.
...
- `cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1393(RepositoryHasNoPythonFiles|PriorityResidualDirectoriesStayPythonFree|GoReplacementPathsRemainAvailable|LaneReportCapturesSweepState)$'`
  Result: `ok  	bigclaw-go/internal/regression	0.596s`
