...
- The repo now also ships a live two-node companion proof in `docs/reports/live-multi-node-subscriber-takeover-report.json`, generated by `scripts/e2e/multi_node_shared_queue.py`.
- Deterministic and live proofs intentionally differ in one boundary: the local harness simulates ownership transitions in-process, while the live proof drives the real lease API in a two-node shared-queue cluster but still routes lease coordination through one node per scenario because the lease backend is not yet shared-durable.
