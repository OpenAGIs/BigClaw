# BIG-GO-177 Python Asset Sweep

## Scope

`BIG-GO-177` delivers `Broad repo Python reduction sweep Y`.

This lane records the remaining physical Python asset inventory for the repository and keeps the priority residual directories Go-only.
...
The Go/native replacement surface that remains available for the retired Python asset areas includes:
...
- `cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO177(RepositoryHasNoPythonFiles|PriorityResidualDirectoriesStayPythonFree|GoReplacementPathsRemainAvailable|LaneReportCapturesSweepState)$'`
  Result: `ok  	bigclaw-go/internal/regression	0.728s`.
