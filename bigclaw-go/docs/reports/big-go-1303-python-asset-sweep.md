# BIG-GO-1303 Python Asset Sweep

## Scope

Heartbeat refill lane `BIG-GO-1303` records the remaining physical Python asset inventory for the repository and keeps the priority residual directories Go-only.
...
- `cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1303(RepositoryHasNoPythonFiles|PriorityResidualDirectoriesStayPythonFree|ReplacementPathsRemainAvailable|LaneReportCapturesSweepState)$'`
  Result: `ok  	bigclaw-go/internal/regression	0.712s`
