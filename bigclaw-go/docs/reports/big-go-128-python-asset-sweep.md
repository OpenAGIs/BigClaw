# BIG-GO-128 Python Asset Sweep

## Scope

Broad repo Python reduction sweep P `BIG-GO-128` records the remaining Python asset baseline with explicit focus on `src/bigclaw`, `tests`, `scripts`, and `bigclaw-go/scripts`.
...
- `cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO128(RepositoryHasNoPythonFiles|PriorityResidualDirectoriesStayPythonFree|ReplacementPathsRemainAvailable|LaneReportCapturesSweepState)$'`
  Result: `ok  	bigclaw-go/internal/regression	1.072s`
