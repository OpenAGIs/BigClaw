...
- `43` Python test modules under `tests/`
- `34` modules directly importing `bigclaw...`
...
- `InventoryPytestAssets(tb)` to machine-check the remaining pytest surface (`43` test modules, `34` `bigclaw` importers, `2` `pytest` importers) instead of leaving that inventory only in prose
...
- `conftest_delete_ready=false blockers=43 legacy pytest modules remain under tests/; 34 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly`
