...
- `49` Python test modules under `tests/`
- `40` modules directly importing `bigclaw...`
...
- `InventoryPytestAssets(tb)` to machine-check the remaining pytest surface (`49` test modules, `40` `bigclaw` importers, `2` `pytest` importers) instead of leaving that inventory only in prose
...
- `49 legacy pytest modules remain under tests/`
- `40 legacy pytest modules still import bigclaw from src/`
...
- `conftest_delete_ready=false blockers=49 legacy pytest modules remain under tests/; 40 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly`
