...
- `37` Python test modules under `tests/`
...
- `InventoryPytestAssets(tb)` to machine-check the remaining pytest surface (`37` test modules, `28` `bigclaw` importers, `2` `pytest` importers) instead of leaving that inventory only in prose
...
- `37 legacy pytest modules remain under tests/`
...
- `conftest_delete_ready=false blockers=37 legacy pytest modules remain under tests/; 28 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly`
...
cd /Users/openagi/code/bigclaw-workspaces/BIG-GO-923/bigclaw-go && go test ./internal/queue ./internal/reporting ./internal/api ./internal/testharness ./cmd/bigclawctl ./internal/regression
cd /Users/openagi/code/bigclaw-workspaces/BIG-GO-923/bigclaw-go && go run ./cmd/bigclawctl pytest-harness --project-root .. --report-path docs/reports/pytest-harness-status.json --json
```
...
- `go test ./internal/queue ./internal/reporting ./internal/api ./internal/testharness ./cmd/bigclawctl ./internal/regression` passed on the latest issue branch state, covering `bigclaw-go/internal/queue/memory_queue_test.go`, `bigclaw-go/internal/reporting/reporting_test.go`, `bigclaw-go/internal/api/server_test.go`, the Go-owned harness smoke, the `pytest-harness` command surface, and the snapshot-alignment regression gate.
- `go run ./cmd/bigclawctl pytest-harness --project-root .. --report-path docs/reports/pytest-harness-status.json --json` passed on the latest issue branch state, regenerated the checked-in JSON snapshot with portable repo-relative paths, and confirmed `inventory_summary=tests=37 bigclaw_imports=28 pytest_imports=2` with `conftest_delete_status.can_delete=false`.
