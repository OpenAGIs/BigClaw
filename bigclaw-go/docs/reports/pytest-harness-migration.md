...
- `46` Python test modules under `tests/`
- `37` modules directly importing `bigclaw...`
...
- `InventoryPytestAssets(tb)` to machine-check the remaining pytest surface (`46` test modules, `37` `bigclaw` importers, `2` `pytest` importers) instead of leaving that inventory only in prose
...
- `conftest_delete_ready=false blockers=46 legacy pytest modules remain under tests/; 37 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly`
