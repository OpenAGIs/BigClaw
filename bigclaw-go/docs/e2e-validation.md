...
```bash
cd bigclaw-go
go run ./cmd/bigclawctl e2e validation-bundle-continuation-scorecard --pretty
```
...
```bash
cd bigclaw-go
go run ./cmd/bigclawctl e2e validation-bundle-continuation-policy-gate --pretty
```
...
```bash
cd bigclaw-go
go run ./cmd/bigclawctl e2e subscriber-takeover-fault-matrix --pretty
```
...
go run ./cmd/bigclawctl e2e run-task-smoke \
  --autostart \
  --go-root "$PWD" \
  --executor local \
  --title "SQLite smoke" \
