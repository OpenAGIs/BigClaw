...
```bash
cd bigclaw-go
go run ./scripts/e2e/validation_bundle_continuation_scorecard --pretty
```
...
```bash
cd bigclaw-go
go run ./scripts/e2e/validation_bundle_continuation_policy_gate --pretty
```
...
```bash
cd bigclaw-go
go run ./scripts/e2e/external_store_validation \
...
```bash
cd bigclaw-go
go run ./scripts/e2e/multi_node_shared_queue \
  --count 200 \
  --submit-workers 8 \
...
```bash
cd bigclaw-go
go run ./scripts/e2e/broker_failover_stub_matrix --pretty
```
...
```bash
cd bigclaw-go
go run ./scripts/e2e/subscriber_takeover_fault_matrix --pretty
```
...
```bash
cd bigclaw-go
go run ./scripts/e2e/cross_process_coordination_surface --pretty
```
...
go run ./scripts/e2e/run_task_smoke \
  --autostart \
  --go-root "$PWD" \
  --executor local \
  --title "SQLite smoke" \
