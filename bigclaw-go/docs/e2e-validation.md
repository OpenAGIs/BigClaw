...
```bash
cd bigclaw-go
scripts/e2e/validation_bundle_continuation_scorecard --pretty
```
...
```bash
cd bigclaw-go
scripts/e2e/validation_bundle_continuation_policy_gate --pretty
```
...
export BIGCLAW_KUBERNETES_IMAGE=alpine:3.20
export BIGCLAW_QUEUE_BACKEND=sqlite
scripts/e2e/mixed_workload_matrix \
...
```bash
cd bigclaw-go
scripts/e2e/external_store_validation \
...
```bash
cd bigclaw-go
scripts/e2e/broker_failover_stub_matrix --pretty
```
...
```bash
cd bigclaw-go
scripts/e2e/subscriber_takeover_fault_matrix --pretty
```
...
```bash
cd bigclaw-go
scripts/e2e/cross_process_coordination_surface --pretty
```
