...
export BIGCLAW_KUBERNETES_IMAGE=alpine:3.20
export BIGCLAW_QUEUE_BACKEND=sqlite
go run ./scripts/e2e/mixed_workload_matrix.go \
...
```bash
cd bigclaw-go
go run ./scripts/e2e/external_store_validation.go \
...
```bash
cd bigclaw-go
go run ./scripts/e2e/multi_node_shared_queue.go \
  --count 200 \
  --submit-workers 8 \
...
```bash
cd bigclaw-go
go run ./scripts/e2e/broker_failover_stub_matrix.go --pretty
```
...
```bash
cd bigclaw-go
go run ./scripts/e2e/subscriber_takeover_fault_matrix.go --pretty
```
...
```bash
cd bigclaw-go
go run ./scripts/e2e/cross_process_coordination_surface.go --pretty
```
