...
```bash
cd bigclaw-go
bash scripts/e2e/validation-bundle-continuation-scorecard --pretty
```
...
```bash
cd bigclaw-go
bash scripts/e2e/validation-bundle-continuation-policy-gate --pretty
```
...
export BIGCLAW_KUBERNETES_IMAGE=alpine:3.20
export BIGCLAW_QUEUE_BACKEND=sqlite
bash scripts/e2e/mixed-workload-matrix \
...
```bash
cd bigclaw-go
bash scripts/e2e/broker-failover-stub-matrix --pretty
```
...
```bash
cd bigclaw-go
bash scripts/e2e/cross-process-coordination-surface --pretty
```
