...
func runLocalIssues(args []string) error {
	...
	switch command {
	case "list":
		return runLocalIssuesList(args[1:])
	case "create":
		return runLocalIssuesCreate(args[1:])
	case "ensure":
		return runLocalIssuesEnsure(args[1:])
	case "set-state":
		return runLocalIssuesSetState(args[1:])
	case "comment":
		return runLocalIssuesComment(args[1:])
	default:
