...
func TestRunPytestHarnessJSONOutput(t *testing.T) {
	...
	if payload["inventory_summary"] != "tests=27 bigclaw_imports=27 pytest_imports=0 pytest_command_refs=0" {
	...
	if deleteStatus["legacy_test_modules"] != float64(27) || deleteStatus["bigclaw_import_modules"] != float64(27) || deleteStatus["pytest_import_modules"] != float64(0) {
	...
	if legacyDeleteStatus["legacy_test_modules"] != float64(27) || legacyDeleteStatus["bigclaw_import_modules"] != float64(27) || legacyDeleteStatus["pytest_import_modules"] != float64(0) || legacyDeleteStatus["pytest_command_refs"] != float64(0) {
		t.Fatalf("unexpected legacy delete status counts: %+v", legacyDeleteStatus)
	}
	if legacyDeleteStatus["summary"] != "legacy_pytest_delete_ready=false blockers=27 legacy pytest modules remain under tests/; 27 legacy pytest modules still import bigclaw from src/" {
