...
func TestRunPytestHarnessJSONOutput(t *testing.T) {
	...
	if payload["inventory_summary"] != "tests=37 bigclaw_imports=28 pytest_imports=2" {
	...
	if deleteStatus["legacy_test_modules"] != float64(37) || deleteStatus["bigclaw_import_modules"] != float64(28) || deleteStatus["pytest_import_modules"] != float64(2) {
...
func TestRunPytestHarnessWritesReportFile(t *testing.T) {
	...
	if deleteStatus["summary"] != "conftest_delete_ready=false blockers=37 legacy pytest modules remain under tests/; 28 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly" {
