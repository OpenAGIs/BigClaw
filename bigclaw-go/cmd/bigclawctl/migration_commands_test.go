...
func TestRunPytestHarnessJSONOutput(t *testing.T) {
	...
	if payload["inventory_summary"] != "tests=0 bigclaw_imports=0 pytest_imports=0 pytest_command_refs=0" {
	...
	if deleteStatus["legacy_test_modules"] != float64(0) || deleteStatus["bigclaw_import_modules"] != float64(0) || deleteStatus["pytest_import_modules"] != float64(0) {
	...
	if legacyDeleteStatus["can_delete"] != true {
		t.Fatalf("expected legacy_pytest_delete_status.can_delete=true, got %+v", legacyDeleteStatus)
	}
	if legacyDeleteStatus["legacy_test_modules"] != float64(0) || legacyDeleteStatus["bigclaw_import_modules"] != float64(0) || legacyDeleteStatus["pytest_import_modules"] != float64(0) || legacyDeleteStatus["pytest_command_refs"] != float64(0) {
		t.Fatalf("unexpected legacy delete status counts: %+v", legacyDeleteStatus)
	}
	if legacyDeleteStatus["summary"] != "legacy_pytest_delete_ready=true blockers=none" {
