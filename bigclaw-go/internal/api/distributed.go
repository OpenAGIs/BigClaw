...
func renderDistributedDiagnosticsMarkdown(diagnostics distributedDiagnostics, filters controlCenterFilters) string {
	...
	lines = append(lines,
		...
		fmt.Sprintf("- Bundle summary: %s", diagnostics.LiveShadowMirror.SummaryPath),
		...
		fmt.Sprintf("- Rollback automation boundary: %s", diagnostics.LiveShadowMirror.RollbackTriggerSurface.AutomationBoundary),
	)
	...
	lines = append(lines,
		...
		fmt.Sprintf("- Live shadow summary: %s", diagnostics.MigrationReviewPack.SummaryPath),
