...
type MigrationPlan struct {
	...
	FirstBatchImplementation []string           `json:"first_batch_implementation"`
	ValidationCommands       []MigrationCommand `json:"validation_commands"`
	RegressionSurface        []string           `json:"regression_surface"`
	BranchRecommendation     string             `json:"branch_recommendation"`
	PRRecommendation         string             `json:"pr_recommendation"`
...
func BuildMigrationPlan() MigrationPlan {
	return MigrationPlan{
		Title: "Workspace bootstrap/cleanup/init migration plan",
		Scope: []string{
		...
		FirstBatchImplementation: []string{
		...
		ValidationCommands: []MigrationCommand{
			{Command: "cd bigclaw-go && go test ./cmd/bigclawctl ./internal/bootstrap", Result: "Covers workspace CLI output and bootstrap/cache lifecycle behavior."},
			{Command: "python -m pytest tests/test_workspace_bootstrap.py tests/test_legacy_shim.py", Result: "Confirms legacy compatibility paths still align with the Go-first workspace flow."},
			{Command: "bash scripts/ops/bigclawctl workspace init --report bigclaw-go/docs/reports/workspace-bootstrap-migration-plan.md", Result: "Refreshes the checked-in migration artifact from the canonical Go command."},
		},
		...
		BranchRecommendation: "Use branch `symphony/BIG-GO-908` and keep all workspace lifecycle migration changes scoped to Go bootstrap surfaces plus migration docs.",
		PRRecommendation:     "Open a PR titled `BIG-GO-908: migrate workspace bootstrap lifecycle to Go commands` and call out the remaining Python modules as frozen compatibility shims, not active implementation.",
		Risks: []string{
...
func RenderMigrationPlanMarkdown(plan MigrationPlan) string {
	lines := []string{
		"# " + plan.Title,
		"",
		"## Scope",
	...
	for _, item := range plan.Scope {
		lines = append(lines, "- "+item)
	}
	lines = append(lines, "", "## First Batch Implementation", "")
	for _, item := range plan.FirstBatchImplementation {
		lines = append(lines, "- "+item)
	}
	lines = append(lines, "", "## Validation Commands", "")
	for _, command := range plan.ValidationCommands {
		lines = append(lines, fmt.Sprintf("- `%s`", command.Command))
		lines = append(lines, fmt.Sprintf("  - %s", command.Result))
	}
	lines = append(lines, "", "## Regression Surface", "")
	for _, item := range plan.RegressionSurface {
		lines = append(lines, "- "+item)
	}
	lines = append(lines, "", "## Branch Recommendation", "", "- "+plan.BranchRecommendation, "", "## PR Recommendation", "", "- "+plan.PRRecommendation, "", "## Risks", "")
...
func WriteMigrationPlan(plan MigrationPlan, path string) (string, error) {
	target := absExpand(path)
	if err := os.MkdirAll(filepath.Dir(target), 0o755); err != nil {
		return "", err
	}
	if hasSuffix(lower(target), ".md") {
		if err := os.WriteFile(target, []byte(RenderMigrationPlanMarkdown(plan)), 0o644); err != nil {
			return "", err
		}
		return target, nil
	}
