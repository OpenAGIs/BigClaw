...
func TestBootstrapWorkspaceCreatesSharedWorktreeFromLocalSeed(t *testing.T) {
	...
	if commonDir := gitOut(t, workspace, "rev-parse", "--git-common-dir"); resolvePath(t, commonDir) != resolvePath(t, filepath.Join(expectedCacheRoot, "seed", ".git")) {
		t.Fatalf("unexpected git common dir %s", commonDir)
	}
	if got := gitOut(t, filepath.Join(expectedCacheRoot, "seed"), "remote", "get-url", "origin"); got != remote {
		t.Fatalf("unexpected origin remote %s", got)
	}
	if got := gitOut(t, filepath.Join(expectedCacheRoot, "seed"), "remote", "get-url", "cache"); resolvePath(t, got) != resolvePath(t, filepath.Join(expectedCacheRoot, "mirror.git")) {
		t.Fatalf("unexpected cache remote %s", got)
	}
}
...
func TestSecondWorkspaceReusesWarmCacheWithoutFullClone(t *testing.T) {
	...
	if first.CacheRoot != second.CacheRoot || !second.CacheReused || !second.CloneSuppressed || second.MirrorCreated || second.SeedCreated || second.WorkspaceMode != "worktree_created" {
		t.Fatalf("unexpected warm-cache reuse result: first=%+v second=%+v", first, second)
	}
}
...
func TestBootstrapWorkspaceReusesExistingIssueWorktree(t *testing.T) {
	...
	if first.Reused || !second.Reused || second.WorkspaceMode != "workspace_reused" || !second.CacheReused || !second.CloneSuppressed || second.Branch != "symphony/OPE-324" {
		t.Fatalf("unexpected reused-workspace status: first=%+v second=%+v", first, second)
	}
}
...
func TestCleanupWorkspacePreservesSharedCacheForFutureReuse(t *testing.T) {
	...
	if !status.Removed || pathExists(workspace) || !pathExists(filepath.Join(cacheRoot, "mirror.git", "HEAD")) || !pathExists(filepath.Join(cacheRoot, "seed", ".git")) || !followUp.CacheReused || !followUp.CloneSuppressed || followUp.MirrorCreated || followUp.SeedCreated {
		t.Fatalf("unexpected cleanup-preservation results: cleanup=%+v followUp=%+v", status, followUp)
	}
}
...
func TestBootstrapRecoversFromStaleSeedDirectoryWithoutRemoteReclone(t *testing.T) {
	...
	if recovered.CacheReused || !recovered.CloneSuppressed || recovered.MirrorCreated || !recovered.SeedCreated || !pathExists(filepath.Join(cacheRoot, "mirror.git", "HEAD")) || !pathExists(filepath.Join(cacheRoot, "seed", ".git")) {
		t.Fatalf("unexpected stale-seed recovery result: %+v", recovered)
	}
}
...
func TestValidationReportCoversThreeWorkspacesWithOneCache(t *testing.T) {
	root := t.TempDir()
	remote := initRemoteWithMain(t, root)
	report, err := BuildValidationReport(remote, filepath.Join(root, "validation-workspaces"), []string{"OPE-272", "OPE-273", "OPE-274"}, "main", "", filepath.Join(root, "repos"), "", true)
	if err != nil {
		t.Fatal(err)
	}
	if report.Summary.WorkspaceCount != 3 ||
		!report.Summary.SingleCacheRootReused ||
		!report.Summary.SingleMirrorReused ||
		!report.Summary.SingleSeedReused ||
		report.Summary.MirrorCreations != 1 ||
		report.Summary.SeedCreations != 1 ||
...
func resolvePath(t *testing.T, path string) string {
	...
	if absErr != nil {
		t.Fatalf("resolve %s: %v / %v", path, err, absErr)
	}
