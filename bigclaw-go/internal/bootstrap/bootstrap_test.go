...
func TestCacheRootForRepoUsesRepoSpecificDirectory(t *testing.T) {
	...
	if got != want {
		t.Fatalf("unexpected cache root: got %s want %s", got, want)
	}
}
...
func TestBootstrapWorkspaceReusesExistingIssueWorktree(t *testing.T) {
	...
	if !second.Reused || second.WorkspaceMode != "workspace_reused" || !second.CacheReused || !second.CloneSuppressed || second.Branch != "symphony/OPE-324" {
		t.Fatalf("expected existing workspace reuse, got %+v", second)
	}
}
...
func TestCleanupWorkspacePreservesSharedCacheForFutureReuse(t *testing.T) {
	...
	if !pathExists(filepath.Join(cacheRoot, "mirror.git", "HEAD")) || !pathExists(filepath.Join(cacheRoot, "seed", ".git")) {
		t.Fatalf("expected cleanup to preserve warm cache under %s", cacheRoot)
	}
...
func TestValidationReportCoversThreeWorkspacesWithOneCache(t *testing.T) {
	...
	if summary.WorkspaceCount != 3 || !summary.SingleCacheRootReused || !summary.SingleMirrorReused || !summary.SingleSeedReused {
		t.Fatalf("unexpected validation summary reuse shape: %+v", summary)
	}
	if summary.MirrorCreations != 1 || summary.SeedCreations != 1 || !summary.CloneSuppressedAfterFirst || !summary.CacheReusedAfterFirst || !summary.CleanupPreservedCache {
		t.Fatalf("unexpected validation summary lifecycle counts: %+v", summary)
	}
}
