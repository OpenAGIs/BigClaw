...
func TestCacheRootForRepoUsesRepoSpecificDirectory(t *testing.T) {
	root := t.TempDir()
	got := CacheRootForRepo("git@github.com:OpenAGIs/BigClaw.git", filepath.Join(root, "repos"), "")
...
func TestBootstrapWorkspaceCreatesSharedWorktreeFromLocalSeed(t *testing.T) {
	...
	expectedCommonDir, err := filepath.EvalSymlinks(filepath.Join(expectedCacheRoot, "seed", ".git"))
	if err != nil {
		t.Fatalf("resolve expected common dir: %v", err)
	}
	gotCommonDir, err := filepath.EvalSymlinks(gitOut(t, workspace, "rev-parse", "--git-common-dir"))
	if err != nil {
		t.Fatalf("resolve actual common dir: %v", err)
	}
	if gotCommonDir != expectedCommonDir {
		t.Fatalf("expected shared git common dir %s, got %s", expectedCommonDir, gotCommonDir)
	}
	if got := gitOut(t, filepath.Join(expectedCacheRoot, "seed"), "remote", "get-url", "origin"); got != remote {
		t.Fatalf("expected seed origin remote %s, got %s", remote, got)
	}
	if got := filepath.Clean(gitOut(t, filepath.Join(expectedCacheRoot, "seed"), "remote", "get-url", "cache")); got != filepath.Clean(filepath.Join(expectedCacheRoot, "mirror.git")) {
		t.Fatalf("expected seed cache remote %s, got %s", filepath.Join(expectedCacheRoot, "mirror.git"), got)
	}
}
...
func TestSecondWorkspaceReusesWarmCacheWithoutFullClone(t *testing.T) {
	...
	if !second.CacheReused || !second.CloneSuppressed || second.MirrorCreated || second.SeedCreated || second.WorkspaceMode != "worktree_created" {
		t.Fatalf("unexpected warm-cache reuse status: %+v", second)
	}
}
...
func TestBootstrapWorkspaceReusesExistingIssueWorktree(t *testing.T) {
	...
	if first.Reused {
		t.Fatalf("expected first bootstrap to be fresh, got %+v", first)
	}
	if !second.Reused || second.WorkspaceMode != "workspace_reused" || !second.CacheReused || !second.CloneSuppressed || second.Branch != "symphony/OPE-324" {
		t.Fatalf("unexpected workspace reuse status: %+v", second)
	}
}
...
func TestBootstrapRecoversFromStaleSeedDirectoryWithoutRemoteReclone(t *testing.T) {
	...
	if recovered.CacheReused || !recovered.CloneSuppressed || recovered.MirrorCreated || !recovered.SeedCreated {
		t.Fatalf("unexpected stale-seed recovery status: %+v", recovered)
	}
	if !pathExists(filepath.Join(cacheRoot, "mirror.git", "HEAD")) || !pathExists(filepath.Join(cacheRoot, "seed", ".git")) {
		t.Fatalf("expected recovered mirror/seed assets under %s", cacheRoot)
	}
}
...
func TestValidationReportCoversThreeWorkspacesWithOneCache(t *testing.T) {
	...
	if report.Summary.WorkspaceCount != 3 || !report.Summary.SingleCacheRootReused || !report.Summary.SingleMirrorReused || !report.Summary.SingleSeedReused {
		t.Fatalf("unexpected validation summary reuse fields: %+v", report.Summary)
	}
	...
	if !report.Summary.CloneSuppressedAfterFirst || !report.Summary.CacheReusedAfterFirst || !report.Summary.CleanupPreservedCache {
		t.Fatalf("unexpected validation summary cache behavior: %+v", report.Summary)
	}
}
