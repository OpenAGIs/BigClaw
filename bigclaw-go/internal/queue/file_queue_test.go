...
func TestFileQueueCreatesParentDirectoryAndPreservesTaskPayload(t *testing.T) {
	...
	if _, err := os.Stat(path); err != nil {
		t.Fatalf("expected queue path to exist: %v", err)
	}
	if len(leased.Labels) != 1 || leased.Labels[0] != "platform" {
		t.Fatalf("expected labels to round trip, got %+v", leased)
	}
	if len(leased.RequiredTools) != 1 || leased.RequiredTools[0] != "browser" {
		t.Fatalf("expected required tools to round trip, got %+v", leased)
	}
	if len(leased.AcceptanceCriteria) != 1 || leased.AcceptanceCriteria[0] != "queue survives restart" {
		t.Fatalf("expected acceptance criteria to round trip, got %+v", leased)
	}
	if len(leased.ValidationPlan) != 1 || leased.ValidationPlan[0] != "pytest tests/test_queue.py" {
		t.Fatalf("expected validation plan to round trip, got %+v", leased)
	}
}
