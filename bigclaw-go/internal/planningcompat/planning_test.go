...
func TestCandidateBacklogRoundTripPreservesManifestShape(t *testing.T) {
	backlog := CandidateBacklog{
		EpicID: "BIG-EPIC-20", Title: "v4.0 v3候选与进入条件", Version: "v4.0-v3",
		Candidates: []CandidateEntry{{
			CandidateID: "candidate-release-control", Title: "Release control center", Theme: "console-governance", Priority: "P0", Owner: "platform-ui",
			...
			EvidenceLinks: []EvidenceLink{{Label: "ui-acceptance", Target: "tests/test_design_system.py", Capability: "release-gate", Note: "role-permission and audit readiness coverage"}},
		}},
	}
	restored, err := designsystem.DeepCopy(backlog)
	if err != nil {
...
func TestCandidateBacklogRanksReadyItemsAheadOfBlockedWork(t *testing.T) {
	backlog := CandidateBacklog{EpicID: "BIG-EPIC-20", Title: "v4.0 v3候选与进入条件", Version: "v4.0-v3", Candidates: []CandidateEntry{
...
func TestEntryGateEvaluationRequiresReadyCandidatesCapabilitiesAndEvidence(t *testing.T) {
	...
	decision := CandidatePlanner{}.EvaluateGate(backlog, gate, &ScopeFreezeAudit{BoardName: "BigClaw v2.0 Freeze", Version: "v2.0", TotalItems: 5})
	if !decision.Passed || !reflect.DeepEqual(map[string]struct{}{"candidate-release-control": {}, "candidate-ops-hardening": {}, "candidate-orchestration": {}}, toSet(decision.ReadyCandidateIDs)) || len(decision.MissingCapabilities) != 0 || len(decision.MissingEvidence) != 0 || !decision.BaselineReady || len(decision.BaselineFindings) != 0 {
...
func TestEntryGateHoldsWhenV2BaselineIsMissingOrNotReady(t *testing.T) {
	...
	failed := CandidatePlanner{}.EvaluateGate(backlog, gate, &ScopeFreezeAudit{BoardName: "BigClaw v2.0 Freeze", Version: "v2.0", TotalItems: 5, MissingValidation: []string{"OPE-116"}})
	if missing.Passed || missing.BaselineReady || !reflect.DeepEqual(missing.BaselineFindings, []string{"missing baseline audit for v2.0"}) || failed.Passed || failed.BaselineReady || !reflect.DeepEqual(failed.BaselineFindings, []string{"baseline v2.0 is not release ready (87.5)"}) {
		t.Fatalf("unexpected baseline gating")
	}
}

func TestEntryGateDecisionRoundTripPreservesFindings(t *testing.T) {
	decision := EntryGateDecision{GateID: "gate-v3-entry", Passed: false, ReadyCandidateIDs: []string{"candidate-release-control"}, BlockedCandidateIDs: []string{"candidate-runtime"}, MissingCapabilities: []string{"commercialization"}, MissingEvidence: []string{"pilot-evidence"}, BaselineReady: false, BaselineFindings: []string{"baseline v2.0 is not release ready (87.5)"}, BlockerCount: 1}
	restored, err := designsystem.DeepCopy(decision)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderCandidateBacklogReportSummarizesBacklogAndGateFindings(t *testing.T) {
	backlog := CandidateBacklog{EpicID: "BIG-EPIC-20", Title: "v4.0 v3候选与进入条件", Version: "v4.0-v3", Candidates: []CandidateEntry{{CandidateID: "candidate-release-control", Title: "Release control center", Theme: "console-governance", Priority: "P0", Owner: "platform-ui", Outcome: "Unify console release gates and promotion evidence.", ValidationCommand: "python3 -m pytest tests/test_design_system.py -q", Capabilities: []string{"release-gate", "reporting"}, Evidence: []string{"acceptance-suite", "validation-report"}, EvidenceLinks: []EvidenceLink{{Label: "ui-acceptance", Target: "tests/test_design_system.py", Capability: "release-gate"}}}}}
...
func TestCandidateEntryRoundTripPreservesEvidenceLinks(t *testing.T) {
	candidate := CandidateEntry{CandidateID: "candidate-ops-hardening", Title: "Ops hardening", Theme: "ops-command-center", Priority: "P0", Owner: "ops-platform", Outcome: "Package command-center and approval surfaces with linked evidence.", ValidationCommand: "python3 -m pytest tests/test_operations.py tests/test_saved_views.py -q", Capabilities: []string{"ops-control", "saved-views"}, Evidence: []string{"weekly-review", "validation-report"}, EvidenceLinks: []EvidenceLink{{Label: "queue-control-center", Target: "src/bigclaw/operations.py", Capability: "ops-control", Note: "queue and approval command center"}, {Label: "saved-view-report", Target: "src/bigclaw/saved_views.py", Capability: "saved-views", Note: "team saved views and digest evidence"}}}
	restored, err := designsystem.DeepCopy(candidate)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFourWeekExecutionPlanRoundTripAndRollups(t *testing.T) {
	plan := BuildBIG4701ExecutionPlan()
	restored, err := designsystem.DeepCopy(plan)
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(restored, plan) || plan.TotalGoals() != 8 || plan.CompletedGoals() != 2 || plan.OverallProgressPercent() != 25 || !reflect.DeepEqual(plan.AtRiskWeeks(), []int{2}) || !reflect.DeepEqual(plan.GoalStatusCounts(), map[string]int{"done": 2, "on-track": 1, "at-risk": 1, "not-started": 4}) {
		t.Fatalf("unexpected plan rollup")
	}
}

func TestFourWeekExecutionPlanValidateRejectsMissingOrUnorderedWeeks(t *testing.T) {
	plan := FourWeekExecutionPlan{PlanID: "BIG-4701", Title: "4周执行计划与周目标", Owner: "execution-office", StartDate: "2026-03-11", Weeks: []WeeklyExecutionPlan{{WeekNumber: 1, Theme: "One", Objective: "One"}, {WeekNumber: 3, Theme: "Three", Objective: "Three"}, {WeekNumber: 2, Theme: "Two", Objective: "Two"}, {WeekNumber: 4, Theme: "Four", Objective: "Four"}}}
...
func TestWeeklyExecutionPlanFlagsAtRiskGoalIDs(t *testing.T) {
	week := WeeklyExecutionPlan{WeekNumber: 2, Theme: "Build and integration", Objective: "Land high-risk integration work.", Goals: []WeeklyGoal{{GoalID: "w2-green", Title: "Green goal", Owner: "eng", Status: "on-track", SuccessMetric: "merged PRs", TargetValue: "2"}, {GoalID: "w2-blocked", Title: "Blocked goal", Owner: "eng", Status: "blocked", SuccessMetric: "open blockers", TargetValue: "0"}}}
	if !reflect.DeepEqual(week.AtRiskGoalIDs(), []string{"w2-blocked"}) {
		t.Fatalf("unexpected at risk ids")
	}
}

func TestBuildV3CandidateBacklogMatchesIssuePlanTraceability(t *testing.T) {
	backlog := BuildV3CandidateBacklog()
	if backlog.EpicID != "BIG-EPIC-20" || backlog.Title != "v4.0 v3候选与进入条件" || !reflect.DeepEqual(candidateIDs(backlog.RankedCandidates()), []string{"candidate-ops-hardening", "candidate-orchestration-rollout", "candidate-release-control"}) {
		t.Fatalf("unexpected built backlog")
	}
	for _, candidate := range backlog.Candidates {
		if !candidate.Ready() {
			t.Fatalf("candidate not ready: %s", candidate.CandidateID)
		}
	}
	ops := findCandidate(backlog.Candidates, "candidate-ops-hardening")
	targets := map[string]struct{}{}
	for _, link := range ops.EvidenceLinks {
		targets[link.Target] = struct{}{}
	}
	for _, target := range []string{"src/bigclaw/operations.py", "tests/test_control_center.py", "tests/test_operations.py", "src/bigclaw/execution_contract.py", "src/bigclaw/workflow.py", "bigclaw-go/internal/workflow/engine_test.go", "bigclaw-go/internal/worker/runtime_test.go", "src/bigclaw/saved_views.py", "tests/test_saved_views.py", "src/bigclaw/evaluation.py", "tests/test_evaluation.py"} {
...
func TestBuildV3EntryGatePassesBuiltCandidateBacklogAgainstV2Baseline(t *testing.T) {
	backlog := BuildV3CandidateBacklog()
	gate := BuildV3EntryGate()
	decision := CandidatePlanner{}.EvaluateGate(backlog, gate, &ScopeFreezeAudit{BoardName: "BigClaw v2.0 Freeze", Version: "v2.0", TotalItems: 25})
	report := RenderCandidateBacklogReport(backlog, gate, decision)
	if !decision.Passed || !reflect.DeepEqual(decision.ReadyCandidateIDs, []string{"candidate-ops-hardening", "candidate-orchestration-rollout", "candidate-release-control"}) || len(decision.MissingCapabilities) != 0 || len(decision.MissingEvidence) != 0 || !strings.Contains(report, "candidate-ops-hardening: Operations command-center hardening") || !strings.Contains(report, "- command-center-src -> src/bigclaw/operations.py capability=ops-control") || !strings.Contains(report, "- report-studio-tests -> tests/test_reports.py capability=commercialization") {
		t.Fatalf("unexpected built gate/report")
	}
}

func candidateIDs(items []CandidateEntry) []string {
	out := make([]string, 0, len(items))
	for _, item := range items {
		out = append(out, item.CandidateID)
	}
	return out
}

func findCandidate(items []CandidateEntry, id string) CandidateEntry {
	for _, item := range items {
