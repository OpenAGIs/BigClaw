...
func TestRepoDiscussionBoardCreateReplyAndFilter(t *testing.T) {
	...
	if comment.CommentID != "repo-"+post.PostID || comment.Anchor != "task:BIG-401" || comment.Status != "open" || !strings.HasPrefix(comment.Body, "Need reviewer") {
...
func TestMergeCollaborationThreadsCombinesNativeAndRepoSurfaces(t *testing.T) {
	...
	board := RepoDiscussionBoard{Now: func() time.Time { return time.Date(2026, 3, 12, 10, 1, 0, 0, time.UTC) }}
	...
	if len(merged.Comments) != 2 || len(merged.Decisions) != 1 {
		t.Fatalf("unexpected merged thread payload: %+v", merged)
	}
