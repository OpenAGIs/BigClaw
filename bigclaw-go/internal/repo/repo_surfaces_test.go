...
func TestRepoRegistryJSONRoundTrip(t *testing.T) {
	registry := RepoRegistry{}
	registry.RegisterSpace(RepoSpace{SpaceID: "space-1", ProjectKey: "BIGCLAW", Repo: "OpenAGIs/BigClaw"})
	...
	if !ok || space.Repo != "OpenAGIs/BigClaw" {
		t.Fatalf("expected space to survive round trip, got %+v %t", space, ok)
	}
	if agent := restored.ResolveAgent("native cloud", "executor"); agent.RepoAgentID != "agent-native-cloud" {
		t.Fatalf("expected cached agent id after round trip, got %+v", agent)
	}
}
