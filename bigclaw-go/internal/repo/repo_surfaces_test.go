...
func TestRepoRegistryJSONRoundTripPreservesSpacesAndAgents(t *testing.T) {
	registry := RepoRegistry{}
	...
	if !ok || space.Repo != "OpenAGIs/BigClaw" {
		t.Fatalf("expected restored space, got %+v ok=%v", space, ok)
	}
	if got := restored.ResolveAgent("native cloud", "executor"); got.RepoAgentID != agent.RepoAgentID {
		t.Fatalf("expected restored agent id %q, got %+v", agent.RepoAgentID, got)
	}
}
