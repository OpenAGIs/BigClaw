...
func TestRepoRegistryJSONRoundTrip(t *testing.T) {
	...
	if !ok || space.Repo != "OpenAGIs/BigClaw" {
		t.Fatalf("expected restored registry space, got %+v ok=%v", space, ok)
	}
	if agent := restored.ResolveAgent("native cloud", "reviewer"); agent.RepoAgentID != "agent-native-cloud" {
