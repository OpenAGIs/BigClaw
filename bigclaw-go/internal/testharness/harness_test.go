...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=51 bigclaw_imports=42 pytest_imports=2" {
	...
	wantSummary := "conftest_delete_ready=false blockers=51 legacy pytest modules remain under tests/; 42 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly"
	...
	wantStatus := ConftestDeletionStatus{
		...
		LegacyTestModules:    51,
		BigclawImportModules: 42,
