...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=30 bigclaw_imports=28 pytest_imports=2" {
	...
	wantSummary := "conftest_delete_ready=false blockers=30 legacy pytest modules remain under tests/; 28 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly"
