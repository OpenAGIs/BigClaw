...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=42 bigclaw_imports=33 pytest_imports=2" {
	...
	wantSummary := "conftest_delete_ready=false blockers=42 legacy pytest modules remain under tests/; 33 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly"
	...
	wantStatus := ConftestDeletionStatus{
		...
		LegacyTestModules:    42,
		BigclawImportModules: 33,
