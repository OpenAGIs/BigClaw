...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=8 bigclaw_imports=8 pytest_imports=0 pytest_command_refs=0" {
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=8 legacy pytest modules remain under tests/; 8 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"8 legacy pytest modules remain under tests/", "8 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    8,
		BigclawImportModules: 8,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
