...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=17 bigclaw_imports=17 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    17,
		BigclawImportModules: 17,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=17 legacy pytest modules remain under tests/; 17 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"17 legacy pytest modules remain under tests/", "17 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    17,
		BigclawImportModules: 17,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
