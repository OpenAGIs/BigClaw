...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=11 bigclaw_imports=11 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    11,
		BigclawImportModules: 11,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=11 legacy pytest modules remain under tests/; 11 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"11 legacy pytest modules remain under tests/", "11 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    11,
		BigclawImportModules: 11,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
