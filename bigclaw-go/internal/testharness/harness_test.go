...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=15 bigclaw_imports=15 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    15,
		BigclawImportModules: 15,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=15 legacy pytest modules remain under tests/; 15 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"15 legacy pytest modules remain under tests/", "15 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    15,
		BigclawImportModules: 15,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
