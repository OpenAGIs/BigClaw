...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=23 bigclaw_imports=23 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    23,
		BigclawImportModules: 23,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=23 legacy pytest modules remain under tests/; 23 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"23 legacy pytest modules remain under tests/", "23 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    23,
		BigclawImportModules: 23,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
