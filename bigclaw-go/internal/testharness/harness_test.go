...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=7 bigclaw_imports=7 pytest_imports=0 pytest_command_refs=0" {
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=7 legacy pytest modules remain under tests/; 7 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"7 legacy pytest modules remain under tests/", "7 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    7,
		BigclawImportModules: 7,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
