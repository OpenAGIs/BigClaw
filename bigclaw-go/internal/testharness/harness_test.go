...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=20 bigclaw_imports=20 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    20,
		BigclawImportModules: 20,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=20 legacy pytest modules remain under tests/; 20 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"20 legacy pytest modules remain under tests/", "20 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    20,
		BigclawImportModules: 20,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
