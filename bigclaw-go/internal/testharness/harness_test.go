...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=9 bigclaw_imports=9 pytest_imports=0 pytest_command_refs=0" {
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=9 legacy pytest modules remain under tests/; 9 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"9 legacy pytest modules remain under tests/", "9 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    9,
		BigclawImportModules: 9,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
