...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=50 bigclaw_imports=41 pytest_imports=2" {
	...
	wantSummary := "conftest_delete_ready=false blockers=50 legacy pytest modules remain under tests/; 41 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly"
	...
	wantStatus := ConftestDeletionStatus{
		...
		LegacyTestModules:    50,
		BigclawImportModules: 41,
