...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=24 bigclaw_imports=24 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    24,
		BigclawImportModules: 24,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=24 legacy pytest modules remain under tests/; 24 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"24 legacy pytest modules remain under tests/", "24 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    24,
		BigclawImportModules: 24,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
