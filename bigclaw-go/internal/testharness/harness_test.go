...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=5 bigclaw_imports=5 pytest_imports=0 pytest_command_refs=0" {
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=5 legacy pytest modules remain under tests/; 5 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"5 legacy pytest modules remain under tests/", "5 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    5,
		BigclawImportModules: 5,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
