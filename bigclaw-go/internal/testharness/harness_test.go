...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=40 bigclaw_imports=31 pytest_imports=2" {
	...
	wantSummary := "conftest_delete_ready=false blockers=40 legacy pytest modules remain under tests/; 31 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly"
	...
	wantStatus := ConftestDeletionStatus{
		...
		LegacyTestModules:    40,
		BigclawImportModules: 31,
