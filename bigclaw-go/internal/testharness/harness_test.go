...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=6 bigclaw_imports=6 pytest_imports=0 pytest_command_refs=0" {
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=6 legacy pytest modules remain under tests/; 6 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"6 legacy pytest modules remain under tests/", "6 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    6,
		BigclawImportModules: 6,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
