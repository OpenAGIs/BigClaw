...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=27 bigclaw_imports=27 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    27,
		BigclawImportModules: 27,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=27 legacy pytest modules remain under tests/; 27 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"27 legacy pytest modules remain under tests/", "27 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    27,
		BigclawImportModules: 27,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
