...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=16 bigclaw_imports=16 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    16,
		BigclawImportModules: 16,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=16 legacy pytest modules remain under tests/; 16 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"16 legacy pytest modules remain under tests/", "16 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    16,
		BigclawImportModules: 16,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
