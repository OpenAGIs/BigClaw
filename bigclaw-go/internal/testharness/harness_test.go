...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=12 bigclaw_imports=12 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    12,
		BigclawImportModules: 12,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=12 legacy pytest modules remain under tests/; 12 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"12 legacy pytest modules remain under tests/", "12 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    12,
		BigclawImportModules: 12,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
