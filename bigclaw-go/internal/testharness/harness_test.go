...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=41 bigclaw_imports=32 pytest_imports=2" {
	...
	wantSummary := "conftest_delete_ready=false blockers=41 legacy pytest modules remain under tests/; 32 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly"
	...
	wantStatus := ConftestDeletionStatus{
		...
		LegacyTestModules:    41,
		BigclawImportModules: 32,
