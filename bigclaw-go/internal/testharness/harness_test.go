...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=45 bigclaw_imports=36 pytest_imports=2" {
	...
	wantSummary := "conftest_delete_ready=false blockers=45 legacy pytest modules remain under tests/; 36 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly"
	...
	wantStatus := ConftestDeletionStatus{
		...
		LegacyTestModules:    45,
		BigclawImportModules: 36,
