...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=14 bigclaw_imports=14 pytest_imports=0 pytest_command_refs=0" {
	...
	wantStatus := ConftestDeletionStatus{
		...
		Summary:              wantSummary,
		Blockers:             []string{},
		LegacyTestModules:    14,
		BigclawImportModules: 14,
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=14 legacy pytest modules remain under tests/; 14 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"14 legacy pytest modules remain under tests/", "14 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    14,
		BigclawImportModules: 14,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
