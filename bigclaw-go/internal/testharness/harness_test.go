...
func TestInventoryPytestAssets(t *testing.T) {
	...
	if got := inventory.Summary(); got != "tests=4 bigclaw_imports=4 pytest_imports=0 pytest_command_refs=0" {
	...
	wantLegacySummary := "legacy_pytest_delete_ready=false blockers=4 legacy pytest modules remain under tests/; 4 legacy pytest modules still import bigclaw from src/"
	...
	wantLegacyStatus := LegacyPytestRetirementStatus{
		...
		Blockers:             []string{"4 legacy pytest modules remain under tests/", "4 legacy pytest modules still import bigclaw from src/"},
		LegacyTestModules:    4,
		BigclawImportModules: 4,
		PytestImportModules:  0,
		PytestCommandRefs:    0,
