...
func TestBIGGO1166CandidatePythonFilesRemainDeleted(t *testing.T) {
	...
	for _, relativePath := range candidates {
		...
		if !errors.Is(err, os.ErrNotExist) {
			t.Fatalf("expected BIG-GO-1166 candidate path to stay deleted: %s (err=%v)", relativePath, err)
		}
	}
}

func TestBIGGO1166MigrationDocsListGoReplacements(t *testing.T) {
	goRepoRoot := repoRoot(t)
	rootRepo := filepath.Clean(filepath.Join(goRepoRoot, ".."))
	...
	requiredGoDoc := []string{
		"Issues: `BIG-GO-902`, `BIG-GO-1053`, `BIG-GO-1160`, `BIG-GO-1166`",
		"## BIG-GO-1166 Sweep Coverage",
	...
	for _, needle := range requiredGoDoc {
		if !strings.Contains(goDoc, needle) {
			t.Fatalf("bigclaw-go/docs/go-cli-script-migration.md missing BIG-GO-1166 replacement %q", needle)
		}
	}

	requiredRootDoc := []string{
		"`BIG-GO-1166` restates the same sweep inventory",
		"`bigclaw-go/scripts/migration/shadow_compare.py`",
		"`scripts/create_issues.py`",
		"`scripts/dev_smoke.py`",
