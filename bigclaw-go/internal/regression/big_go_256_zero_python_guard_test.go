...
func TestBIGGO256RepositoryHasNoPythonFiles(t *testing.T) {
...
func TestBIGGO256SupportAssetSurfacesStayPythonFree(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	...
	for _, relativeDir := range supportDirs {
		...
		if len(pythonFiles) != 0 {
			t.Fatalf("expected support-asset surface to remain Python-free: %s (%v)", relativeDir, pythonFiles)
		}
	}
}

func TestBIGGO256RetainedNativeSupportAssetsRemainAvailable(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO256LaneReportCapturesSweepState(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	report := readRepoFile(t, rootRepo, "bigclaw-go/docs/reports/big-go-256-python-asset-sweep.md")
