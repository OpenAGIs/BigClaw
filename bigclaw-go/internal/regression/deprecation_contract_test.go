...
func TestLegacyMainlineCompatibilityManifestStaysAligned(t *testing.T) {
	...
	if manifest.RuntimeWarning.Surface != "python -m bigclaw (removed)" ||
		manifest.RuntimeWarning.Replacement != "bash scripts/ops/bigclawctl" ||
		!strings.Contains(manifest.RuntimeWarning.Message, "has been removed") ||
	...
	if manifest.ServiceWarning.Surface != "python -m bigclaw serve (removed)" ||
		manifest.ServiceWarning.Replacement != "go run ./bigclaw-go/cmd/bigclawd" ||
		!strings.Contains(manifest.ServiceWarning.Message, "has been removed") ||
