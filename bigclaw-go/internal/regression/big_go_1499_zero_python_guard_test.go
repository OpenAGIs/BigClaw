...
func TestBIGGO1499RepositoryHasNoPythonFiles(t *testing.T) {
...
func TestBIGGO1499PriorityResidualDirectoriesStayPythonFree(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO1499GoOwnershipOrDeleteConditionsAreRecorded(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO1499LaneReportCapturesExplicitLedger(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	report := readRepoFile(t, rootRepo, "bigclaw-go/docs/reports/big-go-1499-python-asset-sweep.md")
	...
	for _, needle := range []string{
		...
		"`cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1499",
	} {
