...
func TestBIGGO1287RepositoryHasNoPythonFiles(t *testing.T) {
...
func TestBIGGO1287PriorityResidualDirectoriesStayPythonFree(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO1287ReplacementPathsRemainAvailable(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO1287LaneReportCapturesSweepState(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	report := readRepoFile(t, rootRepo, "bigclaw-go/docs/reports/big-go-1287-python-asset-sweep.md")
	...
	for _, needle := range []string{
		...
		"`cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1287",
	} {
