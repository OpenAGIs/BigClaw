...
func TestBIGGO1604StandalonePythonRegressionResidueIsRemoved(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	...
	for _, relativePath := range []string{
		...
		if _, err := os.Stat(filepath.Join(rootRepo, filepath.FromSlash(relativePath))); !os.IsNotExist(err) {
			t.Fatalf("expected standalone Python-test regression residue to remain absent: %s (err=%v)", relativePath, err)
		}
	}
}
