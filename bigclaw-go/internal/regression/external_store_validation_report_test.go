...
func TestExternalStoreValidationReportStaysAligned(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			substrings: []string{"go test ./internal/regression -run TestExternalStoreValidationReportStaysAligned -count=1", "external-store-validation-report.json", "backend_matrix", "not_configured", "contract_only"},
		},
		{
