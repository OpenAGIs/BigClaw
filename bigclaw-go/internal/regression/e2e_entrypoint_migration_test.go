...
func TestAutomationScriptSweepCandidatesStayAbsent(t *testing.T) {
	repoRoot := repoRoot(t)
	candidates := []string{
	...
	for _, relative := range candidates {
		if _, err := os.Stat(filepath.Join(repoRoot, relative)); err == nil {
			t.Fatalf("expected migrated automation helper %s to stay absent", relative)
...
func TestAutomationScriptDirectoriesStayPythonFree(t *testing.T) {
	...
	for _, relative := range directories {
		...
		if err != nil {
			if os.IsNotExist(err) && relative == "scripts/migration" {
				continue
			}
		...
		for _, entry := range entries {
			...
			if strings.HasSuffix(entry.Name(), ".py") {
				t.Fatalf("expected no Python helper in %s, found %s", relative, entry.Name())
			}
		}
	}
}
