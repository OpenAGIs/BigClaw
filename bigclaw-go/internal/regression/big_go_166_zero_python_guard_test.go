...
func TestBIGGO166RepositoryHasNoPythonFiles(t *testing.T) {
...
func TestBIGGO166PriorityResidualDirectoriesStayPythonFree(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO166BroadSweepDirectoriesStayPythonFree(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO166NativeReplacementPathsRemainAvailable(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO166LaneReportCapturesSweepState(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	report := readRepoFile(t, rootRepo, "bigclaw-go/docs/reports/big-go-166-python-asset-sweep.md")
	...
	for _, needle := range []string{
		...
		"`cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO166",
	} {
