...
func TestBIGGO1556RepositoryHasNoPythonFiles(t *testing.T) {
...
func TestBIGGO1556WorkspaceBootstrapPlanningResidualAreaStaysPythonFree(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO1556GoReplacementPathsRemainAvailable(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO1556LaneReportCapturesExactLedgerAndBlocker(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	report := readRepoFile(t, rootRepo, "bigclaw-go/docs/reports/big-go-1556-python-asset-sweep.md")
	...
	for _, needle := range []string{
		...
		"`bigclaw-go/internal/planning/planning.go`",
		"`bigclaw-go/internal/api/broker_bootstrap_surface.go`",
		"baseline commit `646edf33` already contains `0`",
		...
		"`cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1556",
	} {
