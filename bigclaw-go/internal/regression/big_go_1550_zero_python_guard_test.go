...
func TestBIGGO1550RepositoryHasNoPythonFiles(t *testing.T) {
...
func TestBIGGO1550WorkspaceBootstrapPlanningResidualAreaStaysPythonFree(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO1550LaneReportCapturesRepoReality(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	report := readRepoFile(t, rootRepo, "bigclaw-go/docs/reports/big-go-1550-python-asset-sweep.md")
	...
	for _, needle := range []string{
		...
		"`cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1550",
	} {
