...
func TestBIGGO1491RepositoryHasNoPythonFiles(t *testing.T) {
...
func TestBIGGO1491PriorityResidualDirectoriesStayPythonFree(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
...
func TestBIGGO1491GoOwnershipAndDeleteConditionsRemainDocumented(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	...
	for _, absentDir := range []string{"src/bigclaw", "tests"} {
		if _, err := os.Stat(filepath.Join(rootRepo, absentDir)); !os.IsNotExist(err) {
			t.Fatalf("expected delete condition to remain satisfied by directory absence: %s", absentDir)
		}
	}
}

func TestBIGGO1491LaneReportCapturesBeforeAfterCounts(t *testing.T) {
	rootRepo := regressionRepoRoot(t)
	report := readRepoFile(t, rootRepo, "bigclaw-go/docs/reports/big-go-1491-python-asset-sweep.md")
	...
	for _, needle := range []string{
		...
		"`cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1491",
	} {
