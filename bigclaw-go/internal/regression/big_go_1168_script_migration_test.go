...
func TestBIGGO1168CandidatePythonFilesRemainDeleted(t *testing.T) {
	goRepoRoot := repoRoot(t)
	rootRepo := filepath.Clean(filepath.Join(goRepoRoot, ".."))

	for _, relativePath := range bigGO1168CandidatePythonFiles() {
		_, err := os.Stat(filepath.Join(rootRepo, filepath.FromSlash(relativePath)))
		if !errors.Is(err, os.ErrNotExist) {
			t.Fatalf("expected BIG-GO-1168 candidate path to stay deleted: %s (err=%v)", relativePath, err)
		}
	}
}

func TestBIGGO1168MigrationDocsListGoReplacements(t *testing.T) {
	goRepoRoot := repoRoot(t)
	rootRepo := filepath.Clean(filepath.Join(goRepoRoot, ".."))
	...
	requiredGoDoc := []string{
		...
		"candidate set locked to zero physical Python files.",
	...
	for _, needle := range requiredGoDoc {
		if !strings.Contains(goDoc, needle) {
			t.Fatalf("bigclaw-go/docs/go-cli-script-migration.md missing BIG-GO-1168 replacement %q", needle)
		}
	}

	requiredRootDoc := []string{
		"`BIG-GO-1168` keeps that sweep active on the current zero-`.py` branch",
		"baseline, revalidates the repo-wide count explicitly",
	...
	for _, needle := range requiredRootDoc {
		if !strings.Contains(rootDoc, needle) {
			t.Fatalf("docs/go-cli-script-migration-plan.md missing BIG-GO-1168 coverage %q", needle)
		}
	}
}

func TestBIGGO1168RepositoryPythonCountRemainsZero(t *testing.T) {
	goRepoRoot := repoRoot(t)
	rootRepo := filepath.Clean(filepath.Join(goRepoRoot, ".."))
	...
	if len(pythonFiles) != 0 {
		t.Fatalf("expected zero physical Python files in repository, found %d: %v", len(pythonFiles), pythonFiles)
	}
}

func bigGO1168CandidatePythonFiles() []string {
	return []string{
