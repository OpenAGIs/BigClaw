...
func TestIssuePriorityArchiveRoundTripPreservesManifestShape(t *testing.T) {
	...
	restored := IssuePriorityArchiveFromMap(archive.ToMap())
	if !reflect.DeepEqual(restored, archive) {
		t.Fatalf("expected archive round-trip to preserve shape, got %+v", restored)
	}
}
...
func TestIssuePriorityArchiveAuditFlagsOwnerPriorityCategoryAndOpenP0Gaps(t *testing.T) {
	...
	audit := Archivist{}.Audit(archive)
	if audit.Ready {
		t.Fatalf("expected ready=false, got %+v", audit)
	}
	if audit.FindingCount != 3 {
		t.Fatalf("expected finding count 3, got %+v", audit)
	}
	...
	if !reflect.DeepEqual(audit.MissingOwners, []string{"BIG-4403-1"}) {
		t.Fatalf("unexpected missing owners: %+v", audit.MissingOwners)
	}
	if !reflect.DeepEqual(audit.InvalidPriorities, []string{"BIG-4403-3"}) {
		t.Fatalf("unexpected invalid priorities: %+v", audit.InvalidPriorities)
	}
	if !reflect.DeepEqual(audit.InvalidCategories, []string{"BIG-4403-3"}) {
		t.Fatalf("unexpected invalid categories: %+v", audit.InvalidCategories)
	}
	if !reflect.DeepEqual(audit.UnresolvedP0Findings, []string{"BIG-4403-1"}) {
		t.Fatalf("unexpected unresolved p0 findings: %+v", audit.UnresolvedP0Findings)
	}
}
...
func TestIssuePriorityArchiveAuditRoundTripAndReadyState(t *testing.T) {
	...
	restored := IssuePriorityArchiveAuditFromMap(audit.ToMap())
	if !reflect.DeepEqual(restored, audit) {
		t.Fatalf("expected audit round-trip to preserve shape, got %+v", restored)
	}
	if got := restored.Summary(); got != "READY: findings=2 missing_owners=0 invalid_priorities=0 invalid_categories=0 unresolved_p0=0" {
		...
		Findings: []ArchivedIssue{
			{
				...
				Status:    "resolved",
				Evidence:  []string{"wireframe-pack"},
			},
			{
				FindingID: "BIG-4403-2",
				...
				Status:    "resolved",
				Evidence:  []string{"review-notes", "permission-checklist"},
			},
		},
	}
