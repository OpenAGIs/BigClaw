...
func (q *ParallelIssueQueue) SaveMarkdown(path string, generatedAt time.Time) (bool, error) {
	absolute, err := queueMarkdownAbsPath(path)
	if err != nil {
		return false, err
	}
	...
	if err := queueMarkdownMkdirAll(dir, 0o755); err != nil {
		return false, err
	}
...
func (q *ParallelIssueQueue) RenderMarkdown(generatedAt time.Time) string {
	...
	out.WriteString("  - `bash scripts/ops/bigclawctl local-issues list`\n")
	out.WriteString(fmt.Sprintf("  - `bash scripts/ops/bigclawctl local-issues set-state --issue BIG-GOM-303 --state %q`\n", activeStateName))
