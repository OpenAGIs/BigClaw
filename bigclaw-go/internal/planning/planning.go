...
func (CandidatePlanner) EvaluateGate(backlog CandidateBacklog, gate EntryGate, baselineAudit *governance.ScopeFreezeAudit) EntryGateDecision {
	...
	blockedCandidates := make([]CandidateEntry, 0, len(backlog.Candidates))
	for _, candidate := range backlog.Candidates {
		if len(candidate.Blockers) > 0 {
			blockedCandidates = append(blockedCandidates, candidate)
		}
	}
	providedCapabilities := make(map[string]struct{})
	providedEvidence := make(map[string]struct{})
...
func (g WeeklyGoal) StatusRank() int {
	return goalStatusOrder[strings.TrimSpace(strings.ToLower(g.Status))]
}
...
func (g WeeklyGoal) IsAtRisk() bool {
	status := strings.TrimSpace(strings.ToLower(g.Status))
...
func joinIntsOrNone(items []int) string {
	if len(items) == 0 {
		return "none"
	}
...
func roundTrip[T any](in T) (T, error) {
	var out T
