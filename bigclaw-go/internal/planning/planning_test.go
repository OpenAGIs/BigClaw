...
func TestCandidateBacklogRanksReadyItemsAheadOfBlockedWork(t *testing.T) {
	backlog := CandidateBacklog{
		...
		Version: "v4.0-v3",
		Candidates: []CandidateEntry{
			{CandidateID: "candidate-risky", Priority: "P0", Capabilities: []string{"runtime-hardening"}, Evidence: []string{"benchmark"}, Blockers: []string{"missing rollback plan"}},
			{CandidateID: "candidate-ready", Priority: "P1", Capabilities: []string{"release-gate", "reporting"}, Evidence: []string{"acceptance-suite", "validation-report"}},
		},
	}
	...
	if !reflect.DeepEqual([]string{ranked[0].CandidateID, ranked[1].CandidateID}, []string{"candidate-ready", "candidate-risky"}) {
		t.Fatalf("unexpected ranked ids: %+v", ranked)
	}
}
...
func TestEntryGateEvaluationRequiresReadyCandidatesCapabilitiesAndEvidence(t *testing.T) {
	backlog := CandidateBacklog{
		...
		Version: "v4.0-v3",
		Candidates: []CandidateEntry{
			{CandidateID: "candidate-release-control", Priority: "P0", Capabilities: []string{"release-gate", "reporting"}, Evidence: []string{"acceptance-suite", "validation-report"}},
			{CandidateID: "candidate-ops-hardening", Priority: "P0", Capabilities: []string{"ops-control"}, Evidence: []string{"weekly-review"}},
			{CandidateID: "candidate-orchestration", Priority: "P1", Capabilities: []string{"commercialization", "handoff"}, Evidence: []string{"pilot-evidence"}},
		},
	}
	gate := EntryGate{
	...
	if !decision.Passed || !decision.BaselineReady {
		t.Fatalf("expected passing decision, got %+v", decision)
	}
	if len(decision.MissingCapabilities) != 0 || len(decision.MissingEvidence) != 0 || len(decision.BaselineFindings) != 0 {
...
func TestEntryGateHoldsWhenV2BaselineIsMissingOrNotReady(t *testing.T) {
	backlog := CandidateBacklog{
		...
		Version: "v4.0-v3",
		Candidates: []CandidateEntry{
			{CandidateID: "candidate-release-control", Priority: "P0", Capabilities: []string{"release-gate"}, Evidence: []string{"acceptance-suite", "validation-report"}},
			{CandidateID: "candidate-ops-hardening", Priority: "P0", Capabilities: []string{"ops-control"}, Evidence: []string{"weekly-review"}},
			{CandidateID: "candidate-orchestration", Priority: "P1", Capabilities: []string{"commercialization"}, Evidence: []string{"pilot-evidence"}},
		},
	}
	gate := EntryGate{
		GateID:                  "gate-v3-entry",
	...
	if missingBaseline.Passed || missingBaseline.BaselineReady || !reflect.DeepEqual(missingBaseline.BaselineFindings, []string{"missing baseline audit for v2.0"}) {
		t.Fatalf("unexpected missing baseline result: %+v", missingBaseline)
	}
	if failedBaseline.Passed || failedBaseline.BaselineReady || !reflect.DeepEqual(failedBaseline.BaselineFindings, []string{"baseline v2.0 is not release ready (87.5)"}) {
		t.Fatalf("unexpected failed baseline result: %+v", failedBaseline)
	}
}
...
func TestBuildV3CandidateBacklogMatchesIssuePlanTraceability(t *testing.T) {
	...
	for _, want := range []string{
		...
		if _, ok := targets[want]; !ok {
			t.Fatalf("missing evidence target %q in %+v", want, opsCandidate.EvidenceLinks)
		}
	}
}
...
func TestBuildV3EntryGatePassesBuiltCandidateBacklogAgainstV2Baseline(t *testing.T) {
	...
	if !decision.Passed {
		t.Fatalf("expected passing decision: %+v", decision)
	}
	...
	if len(decision.MissingCapabilities) != 0 || len(decision.MissingEvidence) != 0 {
		t.Fatalf("unexpected missing items: %+v", decision)
	}
	for _, fragment := range []string{
...
func TestPilotRolloutScorecardAndCandidateGate(t *testing.T) {
	...
	if !strings.Contains(report, "Candidate gate") {
		t.Fatalf("expected candidate gate in report: %s", report)
	}
}
...
func TestRepoWeeklyNarrativeExportsRemainConsistent(t *testing.T) {
	...
	if !strings.Contains(section, "Accepted Commits: 7") || !strings.Contains(exports["markdown"], "Repo Evidence Summary") || !strings.Contains(exports["text"], "Accepted Commits: 7") || !strings.Contains(exports["html"], "<section><h2>Repo Evidence Summary</h2>") {
