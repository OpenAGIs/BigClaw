...
REFRESH_SHARED_QUEUE="${BIGCLAW_E2E_REFRESH_SHARED_QUEUE:-0}"
...
SHARED_QUEUE_REPORT_PATH="${BIGCLAW_E2E_SHARED_QUEUE_REPORT_PATH:-docs/reports/multi-node-shared-queue-report.json}"
SHARED_QUEUE_COUNT="${BIGCLAW_E2E_SHARED_QUEUE_COUNT:-200}"
SHARED_QUEUE_SUBMIT_WORKERS="${BIGCLAW_E2E_SHARED_QUEUE_SUBMIT_WORKERS:-8}"
SHARED_QUEUE_TIMEOUT_SECONDS="${BIGCLAW_E2E_SHARED_QUEUE_TIMEOUT_SECONDS:-180}"
...
if [[ "$REFRESH_SHARED_QUEUE" == "1" ]]; then
  if ! python3 "$ROOT/scripts/e2e/multi_node_shared_queue.py" \
    --go-root "$ROOT" \
    --report-path "$SHARED_QUEUE_REPORT_PATH" \
    --count "$SHARED_QUEUE_COUNT" \
    --submit-workers "$SHARED_QUEUE_SUBMIT_WORKERS" \
    --timeout-seconds "$SHARED_QUEUE_TIMEOUT_SECONDS"; then
    status=1
  fi
fi
