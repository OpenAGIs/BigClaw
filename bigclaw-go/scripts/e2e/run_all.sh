...
if [[ "$RUN_LOCAL" == "1" ]]; then
  (
    BIGCLAW_QUEUE_BACKEND=sqlite \
      python3 "$ROOT/scripts/e2e/run_task_smoke" \
        --autostart \
        --go-root "$ROOT" \
        --executor local \
        --title "SQLite smoke" \
...
export_bundle() {
  ...
  python3 "$ROOT/scripts/e2e/export_validation_bundle" \
    --go-root "$ROOT" \
    --run-id "$RUN_ID" \
...
if [[ "$REFRESH_CONTINUATION" == "1" ]]; then
  python3 "$ROOT/scripts/e2e/validation_bundle_continuation_scorecard" \
    --output "$CONTINUATION_SCORECARD_PATH"

  gate_status=0
  if ! python3 "$ROOT/scripts/e2e/validation_bundle_continuation_policy_gate" \
