#!/usr/bin/env python3
"""Frozen Python report generator pending a Go-native replacement.

Removal condition: delete after a Go command owns capacity certification report
generation and docs point to the Go entrypoint exclusively.
"""
...
GO_MAINLINE_REPLACEMENT = "bigclaw-go/cmd/bigclawctl benchmark capacity-certification"
REMOVAL_CONDITION = (
    "Delete after a Go CLI command replaces this report generator and the "
    "capacity certification docs stop referencing the Python script."
)
...
def build_markdown(report):
    lines = [
        ...
        f"- Default reviewer envelope: `{report['summary']['recommended_sustained_envelope']}`",
        f"- Ceiling reviewer envelope: `{report['summary']['ceiling_envelope']}`",
