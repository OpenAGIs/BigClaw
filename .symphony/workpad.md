...
- update live migration docs so they stop listing the removed Python planning module as an active source surface
...
- live migration docs no longer list `src/bigclaw/planning.py` as an active Python source surface
...
- `rg -n "src/bigclaw/planning.py" docs/go-mainline-cutover-issue-pack.md`

## Validation Results
- `find . -name '*.py' | wc -l` -> `21` after deletion, down from the pre-change baseline `22`
- `cd bigclaw-go && go test ./internal/planning ./internal/regression` -> `ok   bigclaw-go/internal/planning (cached)`; `ok   bigclaw-go/internal/regression (cached)`
...
- `rg -n "src/bigclaw/planning.py" docs/go-mainline-cutover-issue-pack.md` -> exit `1` with no matches
