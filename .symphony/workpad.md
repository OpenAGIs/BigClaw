...
- Add a lane-specific Go regression guard for `BIG-GO-1260` that keeps the repository and the priority residual directories Python-free while asserting the replacement entrypoints still exist.
...
## Acceptance
- The `BIG-GO-1260` lane has an explicit, auditable remaining Python asset inventory.
...
- `cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1260(RepositoryHasNoPythonFiles|PriorityResidualDirectoriesStayPythonFree|ReplacementPathsRemainAvailable|LaneReportCapturesSweepState)$'`
- `git status --short`
