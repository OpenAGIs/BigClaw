...
## Plan
- Extend `bigclaw-go/internal/uigovernance` with the next `UIReviewPack` rendering tranche for dependency, workload, and audit-density surfaces.
- Port the Go helpers and renderers needed for `signoff_dependency_board`, `owner_workload_board`, and `audit_density_board`.
- Reuse the existing Go-native `BuildBIG4204ReviewPack` fixture.
- Remove only the matching Python dependency/workload/density test from `tests/test_reports.py` after the Go-native coverage is in place.
...
## Acceptance
- Scope stays limited to the next `UIReviewPack` dependency/workload/density rendering tranche currently covered by the matching Python tests in `tests/test_reports.py`.
...
- `go test ./internal/uigovernance` (run from `bigclaw-go/`) -> `ok  	bigclaw-go/internal/uigovernance	0.988s`
- `PYTHONPATH=src python3 -m pytest tests/test_reports.py -q` -> `70 passed in 0.28s`
- `wc -l tests/test_reports.py` -> `3160 tests/test_reports.py`
- `git diff --stat -- .symphony/workpad.md bigclaw-go/internal/uigovernance/uigovernance.go bigclaw-go/internal/uigovernance/uigovernance_test.go tests/test_reports.py` -> `.symphony/workpad.md | 17 +-, bigclaw-go/internal/uigovernance/uigovernance.go | 397 ++++++++++++++++++++-, bigclaw-go/internal/uigovernance/uigovernance_test.go | 60 ++++, tests/test_reports.py | 30 --`
