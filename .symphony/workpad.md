...
- attempted to remove `src/bigclaw/__init__.py` by converting `bigclaw` into a namespace package and rewiring `operations.py` to import directly from `runtime.py`, but reverted the change after `PYTHONPATH=src python3` resolved `bigclaw.runtime` to `/Users/openagi/.openclaw/workspace/BigClaw/src/bigclaw/runtime.py` instead of this branch; the explicit package root is still required here to avoid cross-workspace import collisions
