...
## Plan
- Add Go-native repo-collaboration metric coverage in `bigclaw-go/internal/reporting`, then remove the matching Python test from `tests/test_reports.py`.
...
## Acceptance
- Scope stays limited to the repo-collaboration metrics contract migrated out of `tests/test_reports.py`.
...
- `PYTHONPATH=src python3 -m pytest tests/test_reports.py -q` -> `142 passed in 0.43s`
- `go test ./internal/reporting` (run from `bigclaw-go/`) -> `ok  	bigclaw-go/internal/reporting	0.433s`
- `wc -l tests/test_reports.py` -> `5574 tests/test_reports.py`
