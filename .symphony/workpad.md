...
## Plan
- Identify a coherent slice inside `tests/test_reports.py` that already has Go-native replacement coverage.
- Remove the orchestration/workflow-specific Python tests whose behaviors are covered in `bigclaw-go/internal/workflow/orchestration_test.go`.
...
## Acceptance
- Scope stays limited to the orchestration/workflow slice removed from `tests/test_reports.py`.
- Python test coverage for that slice is carried by existing Go-native workflow/orchestration tests.
