...
Current repository Python file count before this sub-batch: `113`
Current `bigclaw-go/scripts/e2e/**` Python file count before this sub-batch: `12`

## Plan

1. Port the `run_all.sh` harness regression coverage from Python to Go.
2. Remove `bigclaw-go/scripts/e2e/run_all_test.py` after the Go replacement is in place.
3. Re-run the adjacent continuation scorecard and gate targeted tests to confirm the harness still exercises the same paths.
...
- Reduce Python files in the targeted directory by removing the selected harness-test batch and replacing it with Go-native paths.
...
- Repository Python files after current sub-batch: `112`
- `bigclaw-go/scripts/e2e/**` Python files before first sub-batch: `15`
- `bigclaw-go/scripts/e2e/**` Python files after current sub-batch: `11`
- Net reduction across this issue so far: `4`
...
- `cd bigclaw-go && go run ./scripts/e2e/validation_bundle_continuation_scorecard.go --output bigclaw-go/docs/reports/validation-bundle-continuation-scorecard.json`
  - Result: exit code `0`
- `cd bigclaw-go && go test ./scripts/e2e/run_all_internal_test.go`
  - Result: `ok  	command-line-arguments	8.211s`
- `git status --short`
