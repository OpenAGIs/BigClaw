...
- Fold the standalone `tests/test_design_system.py` coverage into `tests/test_ui_review.py` and delete the extra physical Python test file while keeping design-system, console IA, and UI review coverage intact.
...
- Fold the standalone `tests/test_planning.py` coverage into `tests/test_reports.py` and delete the extra physical Python test file now that the planning compatibility surface is owned by `reports.py`.
- Fold the remaining standalone `design_system.py` compatibility surface into `ui_review.py`, keep `bigclaw.design_system` and `bigclaw.console_ia` available via package compatibility modules, and delete the extra physical Python file.
...
- `python3 -m py_compile src/bigclaw/ui_review.py src/bigclaw/__init__.py`
...
- Folded the standalone `tests/test_planning.py` coverage into `tests/test_reports.py` and deleted the extra physical Python test file now that the planning compatibility surface is owned by `reports.py`.
...
- Folded the standalone `tests/test_design_system.py` coverage into `tests/test_ui_review.py` and deleted the extra physical Python test file while keeping design-system, console-IA, console-interaction, and UI-review coverage intact.
...
- Folded `src/bigclaw/observability.py` into `src/bigclaw/reports.py`, installed `bigclaw.observability` as a package compatibility module from `src/bigclaw/__init__.py`, updated `src/bigclaw/runtime.py` and `src/bigclaw/operations.py` to consume the merged observability surface from `reports.py`, and deleted the standalone module to reduce repository `.py` count without changing the residual task-run, repo-sync, closeout, or collaboration compatibility surface.
...
- Folded `src/bigclaw/design_system.py` into `src/bigclaw/ui_review.py`, updated `src/bigclaw/__init__.py` so `bigclaw.design_system` and `bigclaw.console_ia` compatibility modules source from `ui_review.py`, and deleted the standalone module to reduce repository `.py` count without changing design-system, console-IA, or UI-review compatibility behavior.
