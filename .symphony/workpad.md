...
## Plan
- Add Go-native benchmark regression-helper coverage in `bigclaw-go/internal/reporting`, then remove the matching Python tests from `tests/test_reports.py`.
...
## Acceptance
- Scope stays limited to the benchmark regression-helper contracts migrated out of `tests/test_reports.py`.
...
- `PYTHONPATH=src python3 -m pytest tests/test_reports.py -q` -> `137 passed in 0.45s`
- `go test ./internal/reporting` (run from `bigclaw-go/`) -> `ok  	bigclaw-go/internal/reporting	0.443s`
- `wc -l tests/test_reports.py` -> `5450 tests/test_reports.py`
