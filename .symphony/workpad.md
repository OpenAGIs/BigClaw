...
## Plan
- confirm the candidate Python sweep list against the materialized worktree and record the pre-change Python-file baseline
- replace the stale active workpad section with this issue's scoped plan, acceptance, and validation before any code changes
- add a regression tranche that locks the lane-owned Python candidate paths to absent-on-disk and asserts concrete Go or shell replacement surfaces for the same benchmark, e2e, migration, and repo-root workflows
- run targeted validation for the new regression tranche, representative Go replacement help paths, and repo-wide Python-count checks
- record exact commands and outcomes, then commit and push the scoped change set

## Acceptance
- the lane-owned candidate Python paths are explicitly covered and remain absent from disk:
...
- concrete Go or shell replacement surfaces are asserted for this lane under `bigclaw-go/cmd/bigclawctl`, `bigclaw-go/scripts`, `scripts/ops`, and migration docs
...
- `cd bigclaw-go && go test ./internal/regression -run TestPhysicalPythonSweepTranche18`
...
- `cd bigclaw-go && go test ./internal/regression -run TestPhysicalPythonSweepTranche18` -> `ok  	bigclaw-go/internal/regression	0.451s`
...
- `git status --short` -> modified `.symphony/workpad.md`; added `bigclaw-go/internal/regression/physical_python_sweep_tranche18_test.go`
