...
1. Remove `tests/test_github_sync.py`, which is already covered by Go github sync tests.
...
3. Run targeted Python/Go validation for the github-sync/harness/report gates, then commit and push.
...
- `cd /Users/openagi/code/bigclaw-workspaces/BIG-GO-923/bigclaw-go && go test ./internal/githubsync ./internal/testharness ./cmd/bigclawctl ./internal/regression`
  Result: passed (`ok  	bigclaw-go/internal/githubsync	4.061s`; `ok  	bigclaw-go/internal/testharness	1.440s`; `ok  	bigclaw-go/cmd/bigclawctl	3.116s`; `ok  	bigclaw-go/internal/regression	2.127s`)
- `cd /Users/openagi/code/bigclaw-workspaces/BIG-GO-923/bigclaw-go && go run ./cmd/bigclawctl pytest-harness --project-root .. --report-path docs/reports/pytest-harness-status.json --json`
  Result: passed (`status=ok`; snapshot uses portable repo-relative paths with `project_root=.` and `conftest_path=tests/conftest.py`; `inventory_summary=tests=39 bigclaw_imports=30 pytest_imports=2`; `conftest_delete_status.can_delete=false`)

## Current Status

- `tests/conftest.py` delete-readiness: `conftest_delete_ready=false blockers=39 legacy pytest modules remain under tests/; 30 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly`
- Structured delete-readiness status:
  `{"can_delete":false,"legacy_test_modules":39,"bigclaw_import_modules":30,"pytest_import_modules":2}`
