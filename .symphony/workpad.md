...
## Plan
- Add Go-native coverage for the remaining workspace cache/worktree lifecycle slice in `bigclaw-go/internal/bootstrap/bootstrap_test.go`.
- Remove the Python workspace lifecycle tests from `tests/test_reports.py` once the equivalent Go tests exist.
...
## Acceptance
- Scope stays limited to the workspace cache/worktree lifecycle slice removed from `tests/test_reports.py`.
