...
- `cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1229(RepositoryHasNoPythonFiles|PriorityResidualDirectoriesStayPythonFree)$'` -> `ok  	bigclaw-go/internal/regression	1.064s`
- `git status --short` -> `captured after final sync`
