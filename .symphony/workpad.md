...
## Plan
- Extend `bigclaw-go/internal/uigovernance` with the next `UIReviewPack` rendering tranche for the owner review queue surface.
- Port only the Go helper and renderer needed for `owner_review_queue` coverage.
- Reuse the existing Go-native `BuildBIG4204ReviewPack` fixture.
- Remove only the matching Python owner-review-queue test from `tests/test_reports.py` after the Go-native coverage is in place.
...
## Acceptance
- Scope stays limited to the next `UIReviewPack` owner-review-queue rendering tranche currently covered by the matching Python tests in `tests/test_reports.py`.
...
- `go test ./internal/uigovernance` (run from `bigclaw-go/`) -> `ok  	bigclaw-go/internal/uigovernance	0.650s`
- `PYTHONPATH=src python3 -m pytest tests/test_reports.py -q` -> `74 passed in 0.30s`
- `wc -l tests/test_reports.py` -> `3315 tests/test_reports.py`
- `git diff --stat -- .symphony/workpad.md bigclaw-go/internal/uigovernance/uigovernance.go bigclaw-go/internal/uigovernance/uigovernance_test.go tests/test_reports.py` -> `.symphony/workpad.md | 19 ++--, bigclaw-go/internal/uigovernance/uigovernance.go | 118 +++++++++++++++++++++, bigclaw-go/internal/uigovernance/uigovernance_test.go | 21 ++++, tests/test_reports.py | 17 ---`
