...
- Add a lane-specific Go regression guard for `BIG-GO-1269` that keeps the repository and the priority residual directories Python-free while asserting the Go or shell replacement entrypoints still exist.
- Add lane reports documenting the remaining Python asset inventory, the replacement paths, and the exact validation commands/results for this sweep.
...
## Acceptance
- The `BIG-GO-1269` lane has an explicit, auditable remaining Python asset inventory.
...
- `cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1269(RepositoryHasNoPythonFiles|PriorityResidualDirectoriesStayPythonFree|ReplacementPathsRemainAvailable|LaneReportCapturesSweepState)$'`
- `git status --short`
