...
## Plan
- Identify the next bounded slice inside `tests/test_reports.py` with direct Go-native workspace-bootstrap coverage.
- Remove the Python workspace bootstrap tests whose behaviors are already covered in `bigclaw-go/internal/bootstrap/bootstrap_test.go`.
...
## Acceptance
- Scope stays limited to the workspace bootstrap/cache slice removed from `tests/test_reports.py`.
