...
## Plan
- Extend the `bigclaw-go/internal/uigovernance` package with the remaining self-contained `UIAcceptance` contracts from `src/bigclaw/design_system.py`.
- Remove the matching Python `UIAcceptance` tests from `tests/test_reports.py` only after the Go-native coverage is in place.
...
## Acceptance
- Scope stays limited to the `UIAcceptance` contracts currently covered by the matching Python tests in `tests/test_reports.py`.
...
- `PYTHONPATH=src python3 -m pytest tests/test_reports.py -q` -> `104 passed in 0.36s`
- `go test ./internal/uigovernance` (run from `bigclaw-go/`) -> `ok  	bigclaw-go/internal/uigovernance	(cached)`
- `wc -l tests/test_reports.py` -> `4426 tests/test_reports.py`
- `git diff --stat` -> `.symphony/workpad.md | 20 +-, bigclaw-go/internal/uigovernance/uigovernance.go | 292 +++++++++++++++++++++, bigclaw-go/internal/uigovernance/uigovernance_test.go | 184 +++++++++++++, tests/test_reports.py | 208 ---------------`
