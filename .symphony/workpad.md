...
- Add a lane-specific Go regression guard for `BIG-GO-1281` that preserves the Python-free repository state and verifies the retained Go or shell replacement entrypoints still exist.
...
## Acceptance
- The `BIG-GO-1281` lane has an explicit remaining Python asset inventory for the repository and priority residual directories.
...
- `cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1281(RepositoryHasNoPythonFiles|PriorityResidualDirectoriesStayPythonFree|ReplacementPathsRemainAvailable|LaneReportCapturesSweepState)$'`
