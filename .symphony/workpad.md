...
3. Remove the migrated Python lane assets once equivalent Go coverage or an explicit delete rationale exists for each file.
...
- `tests/conftest.py`
  - Delete. No Go analogue is required; it only prepended Python `src/` onto `sys.path`.
- `tests/test_runtime.py`
