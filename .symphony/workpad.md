...
- Add a lane-specific Go regression guard for `BIG-GO-1285` that preserves the Python-free repository state and verifies the retained Go or shell replacement entrypoints still exist.
...
- Run the targeted validation commands for this lane, capture the exact results, then commit and push the branch tip to `origin/BIG-GO-1285`.

## Acceptance
- The `BIG-GO-1285` lane has an explicit remaining Python asset inventory for the repository and priority residual directories.
...
- `cd bigclaw-go && go test -count=1 ./internal/regression -run 'TestBIGGO1285(RepositoryHasNoPythonFiles|PriorityResidualDirectoriesStayPythonFree|ReplacementPathsRemainAvailable|LaneReportCapturesSweepState)$'`
