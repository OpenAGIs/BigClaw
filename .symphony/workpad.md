...
## Plan
- confirm the lane-owned benchmark, e2e, migration, and root-script candidate paths from the issue against the actual worktree
- record the repo's pre-change zero-`.py` baseline so the Python-count acceptance blocker is explicit before implementation
- add a scoped regression tranche that locks the lane candidate Python paths to absent-on-disk while asserting the Go-backed automation entrypoints and shell wrappers that replaced them
- validate representative benchmark, e2e, migration, and root compatibility paths through `bigclawctl` help entrypoints
- run targeted validation for the Python-file count baseline and the new regression tranche
...
- `cd bigclaw-go && go test ./internal/regression -run TestTopLevelModulePurgeTranche17` -> `ok  	bigclaw-go/internal/regression	0.480s`
