...
## Plan
- Add Go-native benchmark runner coverage in `bigclaw-go/internal/reporting`, then remove the matching Python tests from `tests/test_reports.py`.
...
## Acceptance
- Scope stays limited to the benchmark runner contracts migrated out of `tests/test_reports.py`.
...
- `PYTHONPATH=src python3 -m pytest tests/test_reports.py -q` -> `127 passed in 0.14s`
- `go test ./internal/reporting` (run from `bigclaw-go/`) -> `ok  	bigclaw-go/internal/reporting	0.458s`
- `wc -l tests/test_reports.py` -> `5211 tests/test_reports.py`
- `git diff --stat` -> `.symphony/workpad.md | 12 +-, bigclaw-go/internal/reporting/reporting.go | 217 ++++++++++++++++++++++++, bigclaw-go/internal/reporting/reporting_test.go | 97 +++++++++++, tests/test_reports.py | 76 ---------`
