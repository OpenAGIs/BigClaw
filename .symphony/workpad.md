...
## Plan
- Add Go-native shared-view rendering coverage in `bigclaw-go/internal/reporting`, then remove the matching Python tests from `tests/test_reports.py`.
...
## Acceptance
- Scope stays limited to the shared-view rendering contracts migrated out of `tests/test_reports.py`.
...
- `PYTHONPATH=src python3 -m pytest tests/test_reports.py -q` -> `134 passed in 0.47s`
- `go test ./internal/reporting` (run from `bigclaw-go/`) -> `ok  	bigclaw-go/internal/reporting	4.062s`
- `wc -l tests/test_reports.py` -> `5386 tests/test_reports.py`
