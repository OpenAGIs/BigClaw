...
1. Delete the isolated Python workflow-definition tranche with Go-owned workflow replacements:
   - `src/bigclaw/dsl.py`
   - `tests/test_dsl.py`
2. Remove stale package exports for the deleted Python DSL surface from `src/bigclaw/__init__.py`.
...
- Repository Python file count decreases from the continuation baseline of `40`.
- `src/bigclaw/dsl.py` is deleted.
- `tests/test_dsl.py` is deleted.
- `src/bigclaw/__init__.py` no longer exports deleted Python-only workflow-definition symbols.
- A Go regression test covers the deletion contract and the Go replacement files for the removed DSL surface.
- Targeted tests pass.
