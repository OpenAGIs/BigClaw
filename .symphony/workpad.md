...
1. Replace `tests/test_live_shadow_bundle.py` with Go-owned regression coverage that exercises the same script behavior, then remove the pytest file.
...
3. Run targeted Python/Go validation for the live-shadow-bundle/harness/report gates, then commit and push.
...
- `cd /Users/openagi/code/bigclaw-workspaces/BIG-GO-923/bigclaw-go && go test ./internal/testharness ./internal/regression`
  Result: passed (`ok  	bigclaw-go/internal/testharness	1.284s`; `ok  	bigclaw-go/internal/regression	1.406s`)
- `cd /Users/openagi/code/bigclaw-workspaces/BIG-GO-923/bigclaw-go && go run ./cmd/bigclawctl pytest-harness --project-root .. --report-path docs/reports/pytest-harness-status.json --json`
  Result: passed (`status=ok`; `inventory_summary=tests=33 bigclaw_imports=28 pytest_imports=2`; `conftest_delete_status.summary=conftest_delete_ready=false blockers=33 legacy pytest modules remain under tests/; 28 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly`)

## Current Status

- `tests/conftest.py` delete-readiness: `conftest_delete_ready=false blockers=33 legacy pytest modules remain under tests/; 28 legacy pytest modules still import bigclaw from src/; 2 legacy pytest modules still import pytest directly`
- Structured delete-readiness status:
  `{"can_delete":false,"legacy_test_modules":33,"bigclaw_import_modules":28,"pytest_import_modules":2}`
