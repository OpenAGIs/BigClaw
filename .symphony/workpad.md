...
## Plan
- Extend `bigclaw-go/internal/uigovernance` with the next `UIReviewPack` rendering tranche for the signoff and escalation dashboard surfaces.
- Port only the Go render helpers and minimal entry builders needed for the `signoff_sla`, `signoff_reminder`, `signoff_breach`, `escalation_dashboard`, `handoff_ledger`, and `owner_digest` coverage.
- Reuse the existing Go-native `BuildBIG4204ReviewPack` fixture for those dashboards.
- Remove only the matching Python dashboard test from `tests/test_reports.py` after the Go-native coverage is in place.
...
## Acceptance
- Scope stays limited to the next `UIReviewPack` signoff/escalation dashboard tranche currently covered by the matching Python tests in `tests/test_reports.py`.
...
- `go test ./internal/uigovernance` (run from `bigclaw-go/`) -> `ok  	bigclaw-go/internal/uigovernance	0.761s`
- `PYTHONPATH=src python3 -m pytest tests/test_reports.py -q` -> `78 passed in 0.32s`
- `wc -l tests/test_reports.py` -> `3385 tests/test_reports.py`
- `git diff --stat -- .symphony/workpad.md bigclaw-go/internal/uigovernance/uigovernance.go bigclaw-go/internal/uigovernance/uigovernance_test.go tests/test_reports.py` -> `.symphony/workpad.md | 20 +-, bigclaw-go/internal/uigovernance/uigovernance.go | 496 +++++++++++++++++++++, bigclaw-go/internal/uigovernance/uigovernance_test.go | 87 ++++, tests/test_reports.py | 39 --`
