...
- `PYTHONPATH=src python3 -m pytest tests`
  ...
  - Re-run after lazy `__main__` alias fix: `251 passed in 4.74s`
- Full-suite side effect:
  - `bigclaw-go/docs/reports/validation-bundle-continuation-policy-gate.json` was regenerated by test coverage outside this issue scope.
  - Restored that file to `HEAD` so the issue branch remains scoped to `src/bigclaw` compression only.
- `python3 - <<'PY'`
  - Result:
    - `src/bigclaw/__init__.py`
    - `src/bigclaw/__main__.py`
