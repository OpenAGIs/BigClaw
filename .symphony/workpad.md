...
## Plan
1. Port `scripts/e2e/multi_node_shared_queue.py` into Go-owned reporting/runtime code plus a small Go entrypoint.
2. Update shared-queue and live-takeover docs/artifacts to point at the Go entrypoint while preserving the checked-in stable evidence values.
3. Add Go coverage for the shared-queue/live-takeover report builders and keep the existing regression surfaces aligned.
...
## Acceptance
- The selected active Python helper (`multi_node_shared_queue.py`) is deleted, not just documented.
