    ...
    backlog = CandidateBacklog(
        ...
        candidates=[
            CandidateEntry(
                ...
                validation_command="cd bigclaw-go && go test ./internal/product ./internal/api",
...
def test_render_candidate_backlog_report_summarizes_backlog_and_gate_findings() -> None:
    ...
    assert "validation=cd bigclaw-go && go test ./internal/product ./internal/api" in report
    assert "- design-system-go-tests -> bigclaw-go/internal/api/expansion_test.go capability=release-gate" in report
...
def test_build_v3_candidate_backlog_matches_issue_plan_traceability() -> None:
    ...
    release_candidate = next(
        candidate for candidate in backlog.candidates if candidate.candidate_id == "candidate-release-control"
    )
