...
def test_benchmark_suite_comparison_and_report(tmp_path: Path) -> None:
    runner = BenchmarkRunner(storage_dir=str(tmp_path))
...
def test_render_candidate_backlog_report_summarizes_backlog_and_gate_findings() -> None:
    ...
    assert "- ui-acceptance -> tests/test_operations.py capability=release-gate" in report
...
def test_build_v3_entry_gate_passes_built_candidate_backlog_against_v2_baseline() -> None:
    ...
    assert "- report-studio-tests -> tests/test_operations.py capability=commercialization" in report
    assert "- console-shell-tests -> tests/test_operations.py capability=release-gate" in report
...
def test_console_interaction_audit_round_trip_preserves_release_findings() -> None:
    draft = build_big_4203_console_interaction_draft()
...
def test_render_console_interaction_report_summarizes_release_readiness() -> None:
    draft = build_big_4203_console_interaction_draft()
    ...
    assert "- Readiness Score: 100.0" in report
    assert "- Release Ready: True" in report
    assert "- Overview: route=/overview required_actions=drill-down, export, audit available_actions=drill-down, export, audit filters=2 states=default, loading, empty, error batch=optional permissions=complete" in report
