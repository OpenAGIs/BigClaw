...
from bigclaw.scheduler import ExecutionRecord, RiskScorer, Scheduler, SchedulerDecision
...
def test_render_and_write_dashboard_builder_report(tmp_path: Path) -> None:
    ...
    report = render_dashboard_builder_report(builder, audit, view=make_ops_shared_view(2))
    report_path = write_dashboard_builder_bundle(str(tmp_path / "dashboard"), builder, audit, view=make_ops_shared_view(2))
...
def test_operations_dashboard_renders_shared_view_loading_state() -> None:
    ...
    dashboard = render_operations_dashboard(snapshot, view=make_ops_shared_view(0, loading=True))
...
def test_regression_center_renders_shared_view_partial_state() -> None:
    ...
    report = render_regression_center(
        center,
        view=make_ops_shared_view(1, partial_data=["Historical baseline fetch is delayed."]),
    )
...
def test_render_policy_prompt_version_center_supports_shared_view_context() -> None:
    ...
    report = render_policy_prompt_version_center(
        center,
        view=make_ops_shared_view(1, partial_data=["Rollback simulation still running."]),
    )
...
def test_queue_persistence_and_priority(tmp_path: Path) -> None:
    qfile = tmp_path / "queue.json"
    queue = PersistentTaskQueue(str(qfile))

    queue.enqueue(Task(task_id="t2", source="linear", title="P1", description="", priority=Priority.P1))
    queue.enqueue(Task(task_id="t1", source="linear", title="P0", description="", priority=Priority.P0))
...
def test_queue_creates_parent_directory_and_preserves_task_payload(tmp_path: Path) -> None:
...
def test_queue_dead_letter_and_retry_persist_across_reload(tmp_path: Path) -> None:
    qfile = tmp_path / "queue.json"
    queue = PersistentTaskQueue(str(qfile))
    queue.enqueue(Task(task_id="t-dead", source="linear", title="dead", description="", priority=Priority.P0))

    task = queue.dequeue_task()
    assert task is not None

    queue.dead_letter(task, reason="executor crashed")
...
def test_queue_loads_legacy_list_storage(tmp_path: Path) -> None:
    qfile = tmp_path / "queue.json"
    qfile.write_text(
        json.dumps(
            [
